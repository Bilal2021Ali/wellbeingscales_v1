...
class Staffs extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        if (!isset($sessiondata) || empty($sessiondata) || $sessiondata['level'] !== 3 || $sessiondata['type'] !== "staff") {
            redirect('EN/users');
            exit();
        } else {
            $userdata = $this->db->get_where('l2_staff', ['Id' => $sessiondata['admin_id']])->result_array();
            if (!empty($userdata)) {
    ...
    public function Dashboard()
    {
        ...
        $this->load->view("EN/staffs/Dashboard");
    ...
    public function Show_surveys()
    {
        ...
        $user_data = $this->db->get_where('l2_staff', array('Id' => $sessiondata['admin_id']), 1)->result_array();
        if (!empty($user_data)) {
        ...
        AND NOT EXISTS (SELECT Id FROM `sv_st1_answers` WHERE `serv_id` = `sv_school_published_surveys`.`Id` AND `user_id` = '" . $sessiondata['admin_id'] . "' AND `sv_st1_answers`.`user_type` = '1' AND `survey_type` = 'fillable' ) 
        AND EXISTS (SELECT Id FROM sv_school_published_fillable_surveys_targetedusers 
        WHERE `sv_school_published_fillable_surveys_targetedusers`.`user_Type` = 'staff' AND `sv_school_published_surveys`.`status` = '1'  
        ...
        JOIN `sv_school_published_surveys` ON sv_school_published_surveys.`Serv_id` = `sv_st1_surveys`.`Id` AND  `sv_school_published_surveys`.`By_school` = " . $school_id . "
        JOIN `sv_school_published_surveys_types` ON `sv_school_published_surveys`.`Id` = `sv_school_published_surveys_types`.`Survey_id` AND `sv_school_published_surveys_types`.`Type_code` = '1'
        JOIN `sv_st_themes` ON `sv_st_themes`.`Id` = `sv_school_published_surveys`.`theme_link`
        ...
        WHERE `sv_school_published_surveys`.`survey_type` = 'notfillable' AND `sv_st1_surveys`.`Status` = '1' AND `sv_school_published_surveys`.`status` = '1'
        AND NOT EXISTS (SELECT Id FROM `sv_st1_answers` WHERE `serv_id` = `sv_school_published_surveys`.`Id` AND `user_id` = '" . $sessiondata['admin_id'] . "' AND `sv_st1_answers`.`user_type` = '1' AND `survey_type` = 'notfillable' ) 
        GROUP BY `sv_school_published_surveys`.`Id` ORDER BY `sv_school_published_surveys`.`Id` DESC  ")->result_array();
        $this->load->view("EN/inc/header", $data);
            ...
            if ($counter_questions == $answers_counter) {
                ...
                if ($this->db->insert('sv_st1_answers', $answers_data)) {
                    ...
                    if ($this->db->insert_batch('sv_st1_answers_values', $answers)) {
                        // $returned_id = $this->db->query("SELECT MAX(Id) as lastadded FROM `sv_st1_answers`
                ...
                echo "please be sure to answer all the questions" . $counter_questions . "  " . $answers_counter;
            }
        }
    }
    ...
    public function Surveys_history()
    {
        ...
        ON `sv_school_published_surveys`.`Id` = `sv_school_published_surveys_types`.`Survey_id` AND `sv_school_published_surveys_types`.`Type_code` = '1'
        JOIN `sv_school_published_surveys_genders`  
        ON `sv_school_published_surveys`.`Id` = `sv_school_published_surveys_genders`.`Survey_id` AND `sv_school_published_surveys_genders`.`Gender_code` = '" . $user_gender . "'
        JOIN sv_st1_answers ON `sv_st1_answers`.`serv_id` =  `sv_school_published_surveys`.`Id`
        WHERE `sv_st1_surveys`.`Status` = '1' AND EXISTS (SELECT Id FROM `sv_st1_answers` WHERE `serv_id` = `sv_school_published_surveys`.`Id` AND `user_id` = '" . $sessiondata['admin_id'] . "' AND `sv_st1_answers`.`user_type` = '1' )
        ...
        JOIN sv_st1_answers ON `sv_st1_answers`.`serv_id` =  `sv_school_published_surveys`.`Id`
        WHERE `sv_school_published_surveys`.`survey_type` = 'fillable' AND `sv_st1_surveys`.`Status` = '1' 
        AND EXISTS (SELECT Id FROM `sv_st1_answers` WHERE `serv_id` = `sv_school_published_surveys`.`Id` AND `user_id` = '" . $sessiondata['admin_id'] . "' AND `sv_st1_answers`.`user_type` = '1' AND `survey_type` = 'fillable' ) 
        AND EXISTS (SELECT Id FROM sv_school_published_fillable_surveys_targetedusers 
        WHERE `sv_school_published_fillable_surveys_targetedusers`.`user_Type` = 'staff'  
        AND `sv_school_published_fillable_surveys_targetedusers`.`user_id` = '" . $sessiondata['admin_id'] . "' 
        AND `sv_school_published_fillable_surveys_targetedusers`.`Survey_id` = `sv_school_published_surveys`.`Id`)
        GROUP BY `sv_school_published_surveys`.`Id` ORDER BY `sv_school_published_surveys`.`Id` DESC  ")->result_array();
        $this->load->view("EN/inc/header", $data);
    ...
    {
        if ($this->uri->segment(4) && $this->uri->segment(5) && is_numeric($this->uri->segment(4)) && md5($this->uri->segment(4)) == $this->uri->segment(5)) {
            $sessiondata = $this->session->userdata('admin_details');
            $data['sessiondata'] = $sessiondata;
            $data['page_title'] = " Qlick Health | your feedback ";
            $data['enter_key'] = $this->uri->segment(4);
            $this->load->view("EN/inc/header", $data);
            $this->load->view("EN/Global/l3_school/give_feedback_of_survey_exp");
            $this->load->view("EN/inc/footer");
        } else {
            $this->load->view('');
        }
    ...
    public function Profile()
    {
        if ($this->input->method() == "get") {
            if ($this->uri->segment(4) && $this->uri->segment(4) == "edite") {
                ...
                $user_data = $this->db->query(" SELECT `l2_staff`.* , Link , `l1_school`.`School_Name_EN` AS Schoolname
                FROM `l2_staff` 
                LEFt JOIN `l2_avatars` ON `l2_avatars`.`For_User` = '" . $sessiondata['admin_id'] . "' AND `l2_avatars`.`Type_Of_User` = 'Staff'
                JOIN `l1_school` ON `l1_school`.`Id` = `l2_staff`.`Added_by` 
                WHERE `l2_staff`.Id = '" . $sessiondata['admin_id'] . "' LIMIT 1 ")->result_array();
                if (!empty($user_data)) {
                ...
                $this->load->view("EN/inc/header", $data);
                $this->load->view("EN/staffs/Profile");
                $this->load->view("EN/inc/footer");
            }
        } else {
            ...
            $this->form_validation->set_rules('Gender', 'Gender', 'trim|required|prep_for_form|encode_php_tags');
    ...
    public function About_us()
    {
        ...
        FROM l3_about_us WHERE `school_id` = '" . $school_id . "' AND `targeted_users` = 'staffs' ")->result_array();
        $this->load->view('EN/inc/header', $data);
    ...
    public function aboutus()
    {
        ...
        $data['sessiondata'] = $sessiondata;
        $this->load->view('EN/inc/header', $data);
        $this->load->view('EN/staffs/aboutus');
        $this->load->view('EN/inc/footer');
    }
}
