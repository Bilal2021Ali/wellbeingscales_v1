    ...
    public function GetClassesList()
    {
        ...
        <script>
            function GetTheStudentsLabResultsForClass(className) {
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url(); ?>EN/ajax/ListLabResultsOfStudents',
                    data: {
                        class: className,
        ...
        if ($method == "POST" && $this->input->post("class") !== "") {
            ...
            foreach ($Ourstaffs as $staff) {
                ...
                foreach ($getResults as $results) {
                    $list[] = array(
                        "Username" => $staffname, "Id" => $ID, "TestId" => $results['Id'], "Testtype" => $results['Device_Test'], "Result" => $results['Result'], "Creat" => $results['Created'], "Symptoms" => $results['Symptoms'], "Time" => $results['Time'], "In" => $results['Action'], 'Grade' => $Grade, "Class_name" => $staff["Class_name"]
                    );
                }
            } ?>
            ...
            <script>
                ...
                $('#student_result_lab_tb').on('click', '.goHome_labtests', function () {
                    var theId = $(this).attr('theId');
                    console.log(theId);
                    Swal.fire({
                        title: 'Do you want to transfer this student to home quarantine?',
                        showDenyButton: true,
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: 'POST',
                                url: '<?= base_url(); ?>EN/Schools/SetStudentInHouse_LabTests',
                                data: {
                                    S_Id: theId,
                                    Action: 'Home',
                                },
                ...
                $('#student_result_lab_tb').on('click', '.Quarantine_labtests', function () {
                    var theId = $(this).attr('theId');
                    console.log(theId);
                    Swal.fire({
                        title: 'Do you want to transfer this student to quarantine?',
                        showDenyButton: true,
                        showCancelButton: true,
                        confirmButtonText: `yes`,
                        cancelButtonText: `cancel`,
...
AND `Test_Description` = '" . $testType . "' ORDER BY `Id` DESC LIMIT 1")->result_array();
                foreach ($getResults as $results) {
                    $list[] = array(
                        ...
                        'position' => $Position_Staff, "UserType" => $u_type,
    ...
    public function Get_Quaranrine_List()
    {
        ...
        foreach ($students as $student) {
            $studentname = $student['F_name_EN'] . ' ' . $student['L_name_EN'];
            $ID = $student['Id'];
...
concat(`F_name_EN`,' ',`L_name_EN`) as Username , `l2_co_patient`.`UserType` ,`l2_co_patient`.`Position` ,
...
ORDER BY `l2_co_labtests`.`TimeStamp` DESC")->result_array();
    ...
    public function Get_home_List_Ministry()
    {
     ...
     `School_Name_EN`,`Id` FROM `l1_school` WHERE `Added_By` = '" . $sessiondata['admin_id'] . "' ORDER BY `Id` DESC ")->result_array();
            //print_r($our_schools);
    ...
    public function Set_ActiononSite()
    {
        if ($this->input->post('S_Id')) {
            ...
            echo " The site paused for cleaning! ";
            ?>
            <script>
                Swal.fire({
    ...
    public function getThisCountrycities()
    {
        if ($this->input->post('id')) {
            ...
            if (!empty($cites)) {
                ...
                echo "Sorry, there are no cities to show at the moment.";
            }
        }
    }
    ...
    private function addcsvStaff($name)
    {
        ...
        if ($s_ar == 13) {
            ...
            foreach ($the_big_array as $array) {
                ...
                ?>
                <script>
                    $('#exportinSatff').html('<?= "Please wait, exporting:" . $array[1] . "   - " . $all; ?>');
                ...
                <?php if ($all == $counter) { ?>
                    <script>
                        Swal.fire({
                            title: 'Success!',
                            text: 'The data was inserted. Succsesfuly.',
                            icon: 'success'
                        });
                        setTimeout(function () {
            ...
            <script>
                Swal.fire({
                    title: " Problem  ",
                    text: 'This is not a csv file for staff',
                    icon: 'warning',
    ...
    private function addcsvTeacher($name)
    {
        ...
        if ($s_ar == 14 && $Position == "Position") {
            ...
            <script>
                Swal.fire({
                    title: " Problem  ",
                    text: 'This is not a csv file for Teachers',
                    icon: 'warning',
    ...
    private function addcsvStudent($name)
    {
        ...
        if ($s_ar == 14 && $the_big_array[0][11] == "Grade") {
            ...
            <script>
                Swal.fire({
                    title: "Problem",
                    text: 'This is not a csv file for students',
                    icon: 'warning',
                    ...
                    if (!empty($alldata)) { ?>
                                ...
                                <div class="card-body">
                                    ...
                                    <table id="Students_table"
                                        ...
                                        <tr>
                                            ...
                                            <th class="text-center"> Blood Oxygen</th>
                        ...
                        <div class="col-md-12">
                            <div class="text-center">
                                ...
                                <h4 class="text-uppercase mt-4"> NO INFORMATION! </h4>
                                <p class="text-muted">Sorry, we could not find any information related to this user.
                                    "<?= $name; ?>"</p>
                            </div>
                        </div>
                        <?php
            ...
            if ($this->form_validation->run()) {
                ...
                if (sizeof($userarr) == 3) {
                    ...
                    if ($type == "Staff" || $type == "Teacher" || $type == "Student") {
                        ...
                        <div class="card">
                            <div class="card-body">
                                <h3 class="text-center">We have an error. Please refresh the page and try again.</h3>
                            </div>
                        </div>
                        <?php
                        exit();
                                    ...
                                    <?php foreach ($alldata as $data) { ?>
                                        <tr>
                                            ...
                                            if ($from > $to) {
                                                ...
                                                $finished_at_s = $timediff->d . ' Days ' . $timediff->h . ':' . $timediff->i . ':' . $timediff->s;
                                            }
                                            ?>
                                            <td><?= $finished_at_s; ?></td>
                                        </tr>
    ...
    public function Attendence_in_of_all()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $To = $this->input->post("End");
            $all_data = array();
            $all_data_staff = $this->db->query(" SELECT l2_staff.Id, l2_staff.F_name_EN, l2_staff.L_name_EN , 
            ...
            $all_data_teacher = $this->db->query("SELECT l2_teacher.Id, l2_teacher.F_name_EN, l2_teacher.L_name_EN , 
            l2_attendance_result.Result_first ,l2_attendance_result.Result_last , 
            l2_attendance_result.Time_first , l2_attendance_result.Time_last , l2_attendance_result.Device_first , 
            l2_attendance_result.Device_last ,l2_attendance_result.Created 
            FROM l2_teacher
            ...
            AND l2_attendance_result.Created <= '" . $To . "' ")->result_array();
            $all_data_student = $this->db->query("SELECT l2_student.Id, l2_student.F_name_EN, l2_student.L_name_EN , 
            l2_attendance_result.Result_first ,l2_attendance_result.Result_last , 
            l2_attendance_result.Time_first , l2_attendance_result.Time_last , l2_attendance_result.Device_first , 
            l2_attendance_result.Device_last ,l2_attendance_result.Created 
            FROM l2_student
            ...
            if (!empty($all_data_staff) || !empty($all_data_teacher) || !empty($all_data_student)) {
                                ...
                                <div class="tab-pane active" id="navpills2-home" role="tabpanel">
                                    <table id="staff_table"
                                        ...
                                        foreach ($all_data_staff as $data) {
                                            ...
                                            <tr>
                                                <td><?= $s_num_s; ?></td>
                                                <td><?= $data['F_name_EN'] . ' ' . $data['L_name_EN']; ?></td>
                                                <td><?= $data['Created'] ?></td>
                                                <td><?= $data['Time_first'] ?></td>
                                                <td><?= $data['Device_first'] ?></td>
                                                <td><?= $data['Result_first'] ?></td>
                ...
                <div class="col-md-12">
                    <div class="text-center">
                        ...
                        <h4 class="text-uppercase mt-4">Sorry, no information was found.</h4>
                        <p class="text-muted">Sorry, we could not find any information for this user. Please enter
                            complete information or you may choose another user.</p>
                    </div>
                </div>
                <?php
            }
            ...
            <?php } else { ?>
                <div class="col-md-12">
                    <div class="text-center">
                        ...
                        <h4 class="text-uppercase mt-4">Sorry, no information was found.</h4>
                        <p class="text-muted">Sorry, we could not find any information for this user. Please enter
                            complete information or you may choose another user.</p><br>
                    </div>
                </div>
            <?php }
        } else {
        ...
        if (
            ...
            if ($this->db->insert('feedBack', $data)) {
                ...
                $config = array(
                    ...
                    'smtp_port' => 465,
                    'smtp_user' => 'no_reply@qlicksystems.com',
    ...
    public function deptclimatedashboard()
    {
        ...
        if ($this->input->post('usertype') && is_numeric($this->input->post('usertype'))) {
            $data['usertype'] = $this->input->post('usertype');
        }
        $data['nofilter'] = true;
        $data['prefix'] = "usertype";
        $data['climate_survyes'] = $this->sv_reports->GetClimatesurveys();
        $this->load->view('EN/Department_comp/inc/climate_dashboard', $data);
    }

    public function chat() // this chat is for Consultant reports page in company/school/ministry/deparment
    {
        $this->load->library('session');
        $sessiondata = $this->session->userdata('admin_details');
        if ($this->input->method() == "post") { // new message to be sent
            if ($this->input->post('message') && is_numeric($this->uri->segment(4))) {
                $message = htmlspecialchars(trim($this->input->post('message')));
                $data = [
                    'sender_id' => $sessiondata['admin_id'],
                    'receiver_id' => $this->input->post('targetKey'),
                    'sender_usertype' => $sessiondata['type'],
                    'receiver_usertype' => $this->input->post('target'),
                    'about' => $this->uri->segment(4),
                    'message_content' => $message
                ];
                if ($this->db->insert('l0_consultant_chat', $data)) { // saving the message
                    return $this->response->json(['status' => "ok"]); // returning the message status
                }
            } else {
                return $this->response->json(['status' => "error", "message" => "Sorry we have an error. Please refresh the page and try again."]);
            }
        } else if ($this->input->method() == "get" && is_numeric($this->uri->segment(4))) { // returns the old messages
            try {
                $this->db->select('*');
                $this->db->from('l0_consultant_chat');
                $this->db->group_start(); // group start
                $this->db->where('sender_id', $sessiondata['admin_id']);
                $this->db->or_where('receiver_id', $sessiondata['admin_id']);
                $this->db->group_end(); // group end
                $this->db->group_start(); // group start
                $this->db->where('sender_usertype', $sessiondata['type']);
                $this->db->or_where('receiver_usertype', $sessiondata['type']);
                $this->db->group_end(); // group end
                $this->db->where('about', $this->uri->segment(4));
                $this->db->order_by('id', 'DESC');
                if ($this->input->get("older") == "true" && $this->input->get("lastId")) {
                ...
                $this->db->reset_query();
                $this->db->select('*');
                $this->db->from("({$subQuery}) AS l0_consultant_chat");
                if ($this->input->get("lastId")) { // gets all the messages after this id to make the load easier
                    if ($this->input->get("older") !== "true") {
                        $lastId = $this->input->get("lastId");
                        $this->db->where('Id >', $lastId);
                    }
                    $group = false;
                    ...
                    $this->db->limit(20); // limited by the last 20 messages on the first load
                }
                $messages = $this->db->get()->result_array();
                // echo $this->db->last_query();
                // getting the recieved messages only
                if (!empty($messages)) {
                    $recieved = array();
                    foreach ($messages as $key => $message) {
                        if ($sessiondata['type'] == "consultant") {
                            if ($message['receiver_id'] == $sessiondata['admin_id'] && $message['sender_usertype'] !== 'consultant' && $message['read_at'] == null) { // received messages condition for consultant
                                $recieved[] = $message['Id'];
                            }
                        } else {
                            if ($message['receiver_id'] == $sessiondata['admin_id'] && $message['sender_usertype'] == 'consultant' && $message['read_at'] == null) { // received messages condition for oth accounts
                                $recieved[] = $message['Id'];
                            }
                        }
                    }
                    if (!empty($recieved)) {
                        // setting as read 
                        $this->db->where_in('Id', $recieved)->set(['read_at' => date('Y-m-d H:i:s')])->update("l0_consultant_chat");
                    }
                }
                // returning the status
                return $this->response->json(['status' => "ok", "messages" => $this->group_by_dates($messages, $group)]);
            } catch (\Throwable $th) {
                return $this->response->json(['status' => "error", "message" => "We Have An Error In Loading The data please retry again later.."]);
            }
        }
    }


    private function group_by_dates($data, $status) // returns the data in groups by the date of the chat
    {
        $result = array();
        foreach ($data as $val) {
            $db = $val['TimeStamp'];
    ...
    {
        if ($this->inpu->post('username')) {
            if (strlen($this->inpu->post('username')) < 3 || strlen($this->inpu->post('username')) > 30) {
                return $this->response->json(['status' => "error", "message" => "The username Must Be between 3 and 30"]);
            }
            // check when everything is ok 👍
            $checkusername = $this->db->where('Username', $this->input->post('username'))->get('v_login')->num_rows();
            if ($checkusername > 0) {
                return $this->response->json(['status' => "error", "message" => "This username is Already In Use"]);
            }
        } else {
            return $this->response->json(['status' => "error", "message" => "Enter A username Please"]);
        }
    }

    public function absence_control()
    {
        if ($this->input->post("userid") && is_numeric($this->input->post("userid")) && $this->input->post("usertype") && in_array($this->input->post("usertype"), ['staff', 'teacher', 'student'])) {
            $userid = $this->input->post("userid");
            $usertype = $this->input->post("usertype");
            $today = date("Y-m-d");
            try {
                $oldRecord = $this->db->where("userid", $userid)->where("usertype", $usertype)->where("day", $today)->get("absence_records");
                $switchedTo = "absent";
                if ($oldRecord->num_rows() > 0) {
                    // update old record
                    $oldStatus = $oldRecord->row();
                    $switchedTo = $oldStatus->undone == 1 ? "absent" : "present";
                    $this->db->where("Id", $oldStatus->Id)->set(["undone" => ($oldStatus->undone == 1 ? 0 : 1)])->update("absence_records");
                    return $this->response->json(['status' => "ok", "to" => $switchedTo]);
                } else {
                    // insert new record
                    $this->db->insert("absence_records", [
                        "userid" => $userid,
            ...
            return $this->response->json(['status' => "error", "message" => "unsupported request..."]);
        }
    }

    public function directorate()
    {
        if (!$this->input->post("country") || !is_numeric($this->input->post("country"))) {
            echo "No Data Found...";
        }

        $data['directorates'] = $this->db->where("Country_Id", $this->input->post("country"))->get("r_directorate")->result_array();
        $data['sub_directorate'] = empty($data['directorates']) ? [] : $this->db->where_in("Directorate_Id", array_column($data['directorates'], "Id"))->get("r_directorate_titles")->result_array();

        $data['default_directorate'] = $this->input->post("directorate") && is_numeric($this->input->post("directorate")) && $this->input->post("directorate") !== 0 ? $this->input->post("directorate") : null;
        $data['default_directorate_type'] = $this->input->post("directorate_type") && is_numeric($this->input->post("directorate_type")) && $this->input->post("directorate_type") !== 0 ? $this->input->post("directorate_type") : null;
        $this->load->view('EN/Component/directorate', $data);
    }

    public function ministry_reports_loader()
    {
        try {
            $this->load->library('session');
            $sessiondata = $this->session->userdata('admin_details');
