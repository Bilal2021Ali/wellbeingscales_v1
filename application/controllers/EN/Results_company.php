...
//print_r($Device_array);
  ...
  public function SelectByPrefix()
  {
    if ($this->input->post("Pref")) {
      ...
      if (empty($list)) { ?>
        <div class="card-body">
          <div class="card-title text-center">Sorry, no data was found.</div>
        </div>
      <?php } else { ?>
        <div class="card-body">
          ...
          <script>
            $(".AddResultPatient").on('focusout', function(e) {
              ...
              $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>EN/Results/AddResultForPatients',
  ...
  private function GetTotal($where, $type, $action = "")
  {
    ...
    foreach ($Ourstaffs as $staff) {
     ...
     AND Created = '" . $today . "' AND UserType = '" . $u_type . "' AND `Result` = '" . $where . "' AND `Test_Description` = '" . $type . "'
...
if($minutes !== 0){
    ...
    <script>
      var chart_by_dept = {
        ...
        series: [{
          ...
          data: [<?php foreach ($ours as $dept) {
                    echo $this->Get_Counter_of_Tests($dept['Id'], $testname, "") . ",";
                  } ?>]
        }, {
          name: "Negative",
          data: [<?php foreach ($ours as $dept) {
                    echo $this->Get_Counter_of_Tests($dept['Id'], $testname, "0") . ",";
                  } ?>]
        }, {
          name: "Positive",
          data: [<?php foreach ($ours as $dept) {
                    echo $this->Get_Counter_of_Tests($dept['Id'], $testname, "1") . ",";
                  } ?>]
        }],
        colors: ["#5b73e8", "#189264", "#C3343C"],
        xaxis: {
          categories: [<?php foreach ($ours as $dept) {
                          echo "'" . $dept['Dept_Name_EN'] . "',";
                        } ?>]
        },
        yaxis: {
          title: {
    ...
    if ($result !== "") {
      foreach ($query_users as $user) {
		...
		`UserType` = '" . $type . "' AND `UserId` = '" . $user['Id'] . "' 
		 AND `Test_Description` = '" . $testDesc . "' AND `Result` = '" . $result . "' AND `Action` = '" . $in . "' LIMIT 1 ")->num_rows();
        $counter += $Results;
      }
    } else {
      foreach ($query_users as $user) {
 ...
 `Dept_Name_EN`,`Id` FROM `l1_co_department` WHERE `Added_By` = '" . $sessiondata['admin_id'] . "' ORDER BY `Id` DESC ")->result_array();
        //print_r($our_schools); 
        ...
        ?>
          ...
          <tr>
            <td style="width: 20px;">
              <?php $avatr_school = $this->db->query("SELECT `Link` FROM `l2_co_avatars` WHERE 
	  `Type_Of_User` = 'Department' AND `For_User` = '" . $school['Id'] . "' ORDER BY `Id` DESC LIMIT 1 ")->result_array(); ?>
              <?php
              ...
              ?>
              <img src="<?php echo base_url(); ?>uploads/avatars/<?php echo $link; ?>" class="avatar-xs rounded-circle " alt="<?php echo $school['Dept_Name_EN'] ?>">
            </td>
            <td>
              <h6 class="font-size-15 mb-1 font-weight-normal"><?php echo $school['Dept_Name_EN']; ?></h6>
            </td>
            <td>
  ...
  ?>
    <table class="table Lab_Table" id="" style="width: 100%;">
      <thead>
        ...
        <th style="color: #ff2e00;border-radius: 12px;margin: 10px;border: 12px solid white;border-radius:30%;padding: 0px;">
          Positive
        </th>
        <th style="color: #1FBD00;border-radius: 12px;margin: 10px;border: 12px solid white;border-radius:30%;padding: 0px;">
          Negative
        </th>
        <th style="color: #0056BD;border-radius: 12px;margin: 10px;border: 12px solid white;border-radius:30%;padding: 0px;text-align: right;">
          Sterilization
        </th>
      </thead>
      <tbody>
