...
?>
            <script>
                ...
                alert('المرجو إعادة تحميل الصفحة');
            </script>
        <?php
            //redirect('AR/users');
            exit();
    ...
    public function notification()
    {
        $this->load->library('session');
        $sessiondata = $this->session->userdata('admin_details');
        $num = $this->db->query("SELECT * FROM `v_notification` WHERE 
    ...
    public function notificationList()
    {
        ...
        $actionlink = base_url("EN/" . $this->ControllerGetter($sessiondata['type']) . ($sessiondata['type'] == "consultant" ? "/chats" : "/Consultant/"));
            ...
            <a href="<?= $actionlink ?>" class="text-reset notification-item">
                <div class="media">
                    ...
                    <div class="media-body">
                        <h6 class="mt-0 mb-1"><?= sizeof($newmessages) ?> رسائل جديدة : <span class="float-right badge rounded-pill bg-danger p-1 text-white">مهم</span></h6>
                        <div class="font-size-12 text-muted">
                            <p class="text-muted">لديك رسائل غير مقروءة</p>
                            <p class="mb-0"><i class="mdi mdi-clock-outline mr-2"></i>تم استلام آخر رسالة في : <?= $newmessages[(sizeof($newmessages) - 1)]['TimeStamp'] ?? "--:--:--" ?></p>
                        </div>
                    </div>
                </div>
            </a>
        ...
        if (!empty($nots)) {
            foreach ($nots as $not) { ?>
                <a href="" class="text-reset notification-item">
                    <div class="media">
                        ...
                        <div class="media-body">
                            <h6 class="mt-0 mb-1">إشعار</h6>
                            <div class="font-size-12 text-muted">
                                <p class="mb-1">"<?php echo $not['User_Entred'] ?>" المستخدم</p>
            ...
            <a href="" class="text-reset notification-item">
                <div class="media">
                    <div class="media-body">
                        <h6 class="mt-0 mb-1">لاتوجد إشعارات</h6>
                    </div>
                </div>
                </div>
            </a>
