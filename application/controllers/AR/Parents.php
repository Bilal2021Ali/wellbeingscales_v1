...
class Parents extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        if (!isset($sessiondata) || empty($sessiondata) || $sessiondata['level'] !== 2 || $sessiondata['type'] !== "Parent") {
            ...
            if (!$sessiondata['regestred'] && $sessiondata['level'] == 2 && $sessiondata['type'] == "Parent" && $link !== "AR/Parents/Register") {
                redirect('AR/Parents/Register');
            }
        }
    }
    ...
    public function index()
    {
        ...
        $this->load->view("AR/Parent/Dashboard");
   ...
   public function Dashboard()
    ...
    public function Select_Child()
    {
        ...
        $data['hasntnav'] = true;
        $this->load->view("AR/inc/header", $data);
        $this->load->view("AR/Parent/Select_Child");
        $this->load->view("AR/inc/footer");
    }
    ...
    public function Register()
    {
        if ($this->input->method() == "get") {
            ...
            $data['hasntnav'] = true;
            $this->load->view("AR/inc/header", $data);
            $this->load->view("AR/Parent/Register");
            $this->load->view("AR/inc/footer");
    ...
    public function SelectByPrefix_co()
    {
        if ($this->input->post("Pref")) {
          ...
          AND Added_By = '" . $Added_By . "' ")->result_array();
                ...
                <?php } else { ?>
                    <div class="card-body">
                        <div class="card-title text-center"> List of Users
                            <hr>
                        </div>
                        <table class="table">
                            <thead>
...
*/
   ...
   public function Home()
    ...
    public function Videos()
    {
        ...
        $this->load->view("AR/inc/header", $data);
        $this->load->view("AR/Parent/Videos");
        $this->load->view("AR/inc/footer");
    }
...
if($minutes !== 0){
    ...
    public function DedicatedSurveys()
    {
        ...
        $this->load->view("AR/inc/header", $data);
        $this->load->view("AR/Global/l3_school/DedicatedSurveyList");
        $this->load->view("AR/inc/footer");
    }
        ...
        if ($this->input->method() == "get") {
            if ($this->uri->segment(4)) {
                ...
                if (!empty($data['survey_data'])) {
                    $data['serv_id'] = $id;
                    $data['students'] = $this->db->query("SELECT Id , CONCAT( F_name_AR , ' ' , L_name_AR ) AS name 
                    ...
                    if ($this->uri->segment(5)) {
                        ...
                        if (empty($answers)) {
                            ...
                            FROM `l2_student` WHERE Id = '" . $student_id . "' ")->result_array()[0] ?? redirect('AR/parents/DedicatedSurveys/' . $id);
                            ...
                            redirect('AR/parents/DedicatedSurvey/' . $id);
                        }
                    }
                    $this->load->view("AR/inc/header", $data);
                    $this->load->view("AR/Global/l3_school/StartDedicatedSurvey");
                    $this->load->view("AR/inc/footer");
                } else {
                    redirect('AR/parents/DedicatedSurveys');
                }
            } else {
                redirect('AR/parents/DedicatedSurveys');
            }
