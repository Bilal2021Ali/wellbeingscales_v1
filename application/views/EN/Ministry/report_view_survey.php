                                    ...
                                    <?php foreach ($used_groups as $group_key => $group_quastion) { ?>
                                        <div class="card mb-1 shadow-none">
                                            ...
                                            <div id="collapse<?= $group_key; ?>" class="collapse show"
                                                 aria-labelledby="heading<?= $group_key; ?>" data-parent="#accordion">
                                                <div class="card-body">
                                                    <?php
                                                    ...
                                                    foreach ($questions as $question_key => $question) { ?>
                                                        ...
                                                        <div class="card">
                                                            <div class="card-body quastions-title">
                                                                <h3 class="card-title"><?= $questions_counter ?>
                                                                    . <?= $question['en_title'] ?>
                                                                    <span class="open_more_q float-right"
                                                                          link="<?= base_url('EN/DashboardSystem/question_detailed_report/' . $serv_id . "/" . $question['q_id'] . "/" . $group_choices . '/' . md5(time())) ?>"
                                                        ...
                                                        <div class="row px-2">
                                                            <?php foreach ($choices as $key => $choice) { ?>
                                                                <?php
                                                                $use_count = $this->db->query(" SELECT Id FROM `sv_st1_answers_values` WHERE `question_id` = '" . $question['q_id'] . "' AND `choice_id` = '" . $choice['Id'] . "' ")->num_rows();
                                                                $all_count = $this->db->query(" SELECT Id FROM `sv_st1_answers_values` WHERE `question_id` = '" . $question['q_id'] . "'  ")->num_rows();
                                                                ?>
                                                                <div class="card col">
                                                                    <div class="card-body">
                                                                        <div class="mr-2">
                                                                            <label><?= $choice['title_en'] ?>
                                                                                ( <?= calc_perc($use_count, $all_count); ?>
                                                                                )</label>
                                                                            <span class="open_moreD"
                                                                                  link="<?= base_url('EN/DashboardSystem/question_choice_report/' . $serv_id . "/" . $choice['Id'] . "/" . $question['q_id'] . "/" . calc_perc($use_count, $all_count) . '/' . md5(time())) ?>"
                                                                                  target="_blank"
                                                ...
                                                <div class="row px-2">
                                                    <?php foreach ($choices as $key => $choice) { ?>
                                                        ...
                                                        <div class="card col">
                                                            <div class="card-body">
                                                                <div class="mr-2">
                                                                    <label><?= $choice['title_en'] ?></label> <br>
                                                                    <label>(<?= calc_perc($use_count, $all_count); ?>
                                                                        )</label>
                                                                    <span class="open_moreD"
                                                                          link="<?= base_url('EN/DashboardSystem/question_choice_report/' . $serv_id . "/" . $choice['Id'] . "/" . $question['q_id'] . "/" . calc_perc($use_count, $all_count) . '/' . md5(time())) ?>"
