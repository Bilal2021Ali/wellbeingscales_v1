            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-12 select-systemes-container">
                        <h3 class="card-title mb-5"><?= ucfirst($type) ?> list:

                            <button disabled type="button" class="btn btn-success waves-effect waves-light float-right upload-report">
                                <i class="uil-upload mr-2"></i><span class="mr-2">|</span> Upload Report
                            </button>
                        </h3>
                                    ...
                                    <?php foreach ($systemes as $sn => $systeme) { ?>
                                        <tr id="systeme-upload-<?= $systeme['Id'] ?>">
                                            ...
                                            <td class="text-center">
                                                <button data-toggle="tooltip" data-placement="top" title="Upload" class="btn btn-rounded waves btn-success single-upload" data-key="<?= $systeme['Id'] ?>"><i class="uil uil-upload"></i></button>
                                                <button data-toggle="tooltip" data-placement="top" title="Show The Uploaded Files" class="btn btn-rounded waves btn-warning show-uploads" data-key="<?= $systeme['Id'] ?>"><i class="uil uil-file"></i></button>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                    ...
                    <div class="col-4 hidden selected-systemes-container">
                        <h3 class="card-title mb-4">Your Selected <?= ucfirst($type) ?> :</h3>
            ...
            <div class="modal-dialog">
                <form class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="upload-new-reportLabel">Report Upload:</h5>
                    ...
                    <div class="modal-body">
                        <div class="modal-body text-center">
                            ...
                            <h3 class="mt-1 text-truncate">Click to select a file</h3>
                            <label class="float-left">Comments</label>
                            <textarea class="form-control" rows="3" name="comments" placeholder="Comments">No Comment</textarea>
                        </div>
                    </div>
...
<script>
    ...
    const RenderSelectedList = (checkedValues) => {
        ...
        if (checkedValues.length > 0) {
            ...
            $('.selected-systemes-container h3').html("Your Selected Schools :");
    ...
    $('#upload-new-report form input[name="newFile"]').change(function(e) {
        $('#upload-new-report form h3').html('Selected File :' + e.target.files[0].name);
    });

    $('#upload-new-report form').on('submit', function(e) {
        e.preventDefault();
        $('#upload-new-report button[type="submit"]').attr('disabled', '');
        $('#upload-new-report button[type="submit"]').html('Please wait...');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#upload-new-report button[type="submit"]').html('Save Changes');
                if (response.status == "ok") {
        ...
        $('#upload-new-report form h3').html('Click to select a file');
    });
    ...
    $('.table').on('click', '.show-uploads', function() {
        ...
        $('.selected-systemes-container h3').html("Uploaded Reports :");
        $.ajax({
            type: "GET",
            url: "<?= current_url() ?>/" + id,
            success: function(response) {
                ...
                if (response.status = "ok") {
                    if (response.files.length > 0) {
                        response.files.forEach(file => {
                            $('.selected-systemes').append(`<div class="card shadow-none">
                                        <div class="card-body">
                                            <a href="${file.link}"><p class="mb-0 font-weight-bold text-dark">The File Link</p></a>
                                            <span class="font-weight-light text-muted">${file.Comments || "No Comment"}</span>
                                            <i data-toggle="tooltip" data-placement="top" title="Delete This File" data-key="${file.Id}" class="uil uil-trash float-right font-size-20 text-danger delete-report"></i>
                                            ${file.ChatMessages > 0 ? '<a href="<?= base_url("EN/Consultant/Chat/") ?>'+ file.Id +'"><i data-toggle="tooltip" data-placement="top" title="Chat" class="uil uil-message float-right font-size-20 text-warning report-chat"></i></a>' : '' }
                                        </div>
                                    </div>`);
                        });
                    } else {
                        $('.selected-systemes').html('No Files Found !!');
                    }
                } else {
                    Swal.fire({
                        title: "error",
