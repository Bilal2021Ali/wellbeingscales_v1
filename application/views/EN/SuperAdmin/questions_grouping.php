...
<body>
    <div class="main-content">
        ...
        <div class="page-content"><!-- AddGroup -->
		 <h4 class="card-title" style="background: #800080; padding: 10px;color: #ffffff;border-radius: 4px;">SU 015: Questions Grouping for All Surveys</h4>
            ...
            <div class="row">
                <?php foreach ($used_groups as $key => $group) { ?>
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body" >          
                                <h3 class="card-title"><?php echo $group['title_en'] ?>
                                    <button data-toggle="modal" data-target="#exampleModal" onclick="addToThisGroup(<?php echo $group['Id']; ?>)" style="width: 30px;height: 30px;border-radius: 100%;padding: 1px;" class="btn btn-ronded float-right"><i class="uil uil-plus"></i></button>
                                    <button data-toggle="modal" data-target="#editetitleGroup" onclick="chnageid(<?php echo $group['Id']; ?>)" style="width: 30px;height: 30px;border-radius: 100%;padding: 1px;" class="btn btn-ronded float-right"><i class="uil uil-edit"></i></button>
                                    <button onclick="deletegroup(<?php echo $group['Id']; ?>)" style="width: 30px;height: 30px;border-radius: 100%;padding: 1px;" class="btn btn-ronded float-right"><i class="uil uil-trash"></i></button>
                                </h3>
                                <br>
                                <div id="accordion_<?php echo $key; ?>" class="custom-accordion group_content" data-simplebar="init">
                                    <?php
                                    if($surv_type == "fillable"){
                                        $questions_of_grop  = $this->db->query("SELECT 
                                        `sv_st_fillable_questions`.*, `sv_questions_library`.*, `sv_st_fillable_questions`.`Id` AS Id FROM 
                                        `sv_st_fillable_questions`
                                        INNER JOIN `sv_questions_library` ON `sv_questions_library`.`Id` = `sv_st_fillable_questions`.`question_id`
                                        WHERE `sv_st_fillable_questions`.`Group_id` = '" . $group['Id'] . "' ")->result_array();
                                    }else{
                                        $questions_of_grop  = $this->db->query("SELECT 
                                        `sv_st_questions`.*, `sv_questions_library`.*, `sv_st_questions`.`Id` AS Id FROM 
                                        `sv_st_questions`
                                        INNER JOIN `sv_questions_library` ON `sv_questions_library`.`Id` = `sv_st_questions`.`question_id`
                                        WHERE `sv_st_questions`.`Group_id` = '" . $group['Id'] . "' ")->result_array();
                                    }
                                    ?>
                                    <?php if(!empty($questions_of_grop)){ ?>
                                        <?php foreach ($questions_of_grop as $sn => $questions) { ?>
                                            <div class="card mb-1 shadow-none notstatic">
                                                <a href="#collapse_<?php echo $questions['Id'] ?>" class="text-dark" data-toggle="collapse" aria-expanded="true" aria-controls="collapse_<?php echo $questions['Id'] ?>">
                                                    <div class="card-header" id="<?php echo 'Hd_' . $questions['Id'] . '__' . $sn ?>">
                                                        <h6 class="m-0">
                                                            <?php echo $questions['en_title']; ?>
                                                            <i class="mdi mdi-chevron-up float-right accor-down-icon"></i>
                                                            <i class="uil-share float-right mr-1" data-toggle="modal" data-target="#change_the_group" onclick="transferequestion(<?php echo $group['Id']; ?>,<?php echo $questions['Id']; ?>)"></i>
                                                        </h6>
                                                    </div>
                                                </a>
                                                <div id="collapse_<?php echo $questions['Id'] ?>" class="collapse <?php echo $sn == 0 ? "show" : "" ?>" aria-labelledby="<?php echo 'Hd_' . $questions['Id'] . '__' . $sn ?>" data-parent="#accordion_<?php echo $key; ?>" style="">
                                                    <div class="card-body">
                                                        <?php echo $questions['en_desc']; ?>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php } ?>
                                    <?php }else{ ?>
                                        <div class="empty">
                                            <img src="<?php echo base_url('assets/images/empty.svg') ?>" class="empty" alt="">
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <form id="transfere_to">
                                ...
                                <input type="hidden" name="__serv__" value="<?php echo $group_id ?>">
            ...
            <?php }else{ ?>
                <div class="row">
                    ...
                    <div class="col-lg-6 offset-lg-3 text-center pt-2">
                            <img src="<?php echo base_url('assets/images/empty.svg') ?>" class="w-100 mb-1" alt="">
...
function chnageid(id) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>EN/Dashboard/add_questions_to_group?survtype=<?= $surv_type ?>',
        data: {
            serv_id  : <?php echo $group_id ?>,
            group_id : id,
            for      : 'titles_get',
        },
        beforeSend: function() {
...
function addToThisGroup(id) {
    ...
    $.ajax({
        type: 'POST',
        url: '<?php echo base_url(); ?>EN/Dashboard/questions_grouping?survtype=<?= $surv_type ?>',
        data: {
            serv_id: <?php echo $group_id ?>,
            group_id: id,
        },
