...
$new_to_connect = $this->db->query("SELECT DISTINCT(`l1_co_department`.Id),`Dept_Name_EN` AS EN_Title 
FROM `l1_co_department`
LEFT JOIN `v0_area_device_permission` ON `v0_area_device_permission`.`system_id` != `l1_co_department`.`Id` 
AND `v0_area_device_permission`.`relation_type` = '1' WHERE `l1_co_department`.`id` != '".$ref_data['Dept_id']."' ")->result_array();
	  
$connecteds = $this->db->query("SELECT 
`l1_co_department`.`Dept_Name_EN` AS dept_title ,
...
AND `v0_area_device_permission`.`relation_type` = '1'
JOIN  `l1_co_department` ON `l1_co_department`.`Id` = `v0_area_device_permission`.`system_id`
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title mt-0" id="myModalLabel">connect with system</h5>
			...
			<div class="card ">
				<div class="card-body">
					<div class="text-center">
						<div class="mb-4">
							<img src="<?php echo base_url() ?>uploads/avatars/<?php  echo $ref_data['Dept_puc'];  ?>" alt="" 
					...
					<div class="">
						<p><strong>Mac adress : </strong><?php  echo $ref_data['mac_adress']; ?></p>
						<p><strong>type : </strong><?php  echo $ref_data[ 'device_name' ] . '(' . $ref_data[ 'min_temp' ] . ' / ' . $ref_data[ 'max_temp' ] . ')';  ?></p>
						<p><strong>the site : </strong><?php  echo $ref_data['The_Site_Name']  ?></p>
						<p><strong>Date &amp; Time : </strong><?php  echo $ref_data['Added_in']  ?></p>
					</div>
					<button type="button" class="btn btn-primary btn-lg btn-block waves-effect waves-light mb-1 add"
