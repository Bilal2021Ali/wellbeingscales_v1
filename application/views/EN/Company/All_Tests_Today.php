...
WHERE `Added_By` = '" . $sessiondata['admin_id'] . "' ORDER BY `Id` DESC ")->result_array();
				...
				function data_of_user($type, $sessiondata)
				{
					...
					$ours = $ci->db->query("SELECT 
`Dept_Name_EN`,`Manager_EN`,`Id` FROM `l1_co_department` 
WHERE `Added_By` = '" . $sessiondata['admin_id'] . "' ORDER BY `Id` DESC ")->result_array();

					foreach ($ours as $dept) {
						$Ourstaffs = $ci->db->query("SELECT * FROM l2_co_patient WHERE
`Added_By` = '" . $dept['Id'] . "' ")->result_array();
						$Position_Staff	= $dept['Dept_Name_EN'] . ' , ' . $dept['Manager_EN'];
...
AND Created = '" . $today . "' AND UserType = '" . $type . "' ORDER BY `TimeStamp` DESC LIMIT 1")->result_array();
				...
				<div class="row formcontainer" id="Staff_list">
					...
					<div class="col-12">
					<h4 class="card-title" style="background: #0eacd8;padding: 10px;color: #f5f6f8;border-radius: 4px;"> CO013: User Temperature in <?= $sessiondata['f_name']; ?> </h4>
						<div class="card">
							<div class="card-body">
