...
<div class="main-content">
	...
	<h4 class="card-title" style="background: #0eacd8;padding: 10px;color: #f5f6f8;border-radius: 4px;"> CO017: REFRIGERATORS TRIP REPORT </h4>
                    ...
                    <form class="row mb-3" method="POST">
                        <div class="col-lg-4">
                            ...
                            <select name="department" class="form-control">
                                <option value="" <?= $department == "" ? "selected" : "" ?>>All</option>
                                <?php foreach ($depts as $dept) { ?>
                                    <option value="<?= $dept['Id'] ?>" <?= $department == $dept['Id'] ? "selected" : "" ?>><?= $dept['Dept_Name_EN'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-lg-4">
...
<script>
    $('select[name="department"]').change(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>EN/Company/machenesForDepartment',
            data: {
                dept_id: id,
            },
    ...
    $('select[name="machine"]').change(function() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>EN/Company/tripNames_forthisMachine',
            data: {
                machine_id: id,
            },
            success: function(data) {
