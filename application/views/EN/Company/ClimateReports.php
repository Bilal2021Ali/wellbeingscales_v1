...
<div class="main-content">
    <div class="page-content">
        ...
        <?php foreach ($types as $type) { ?>
            <div class="w-100 reportschartsfor_<?= $type['Id'] ?>">
                <div class="card">
                    <div class="card-body">
                        <h2 class="card-title" style="background: #52d155; padding: 20px;color: #1E1E1E;border-radius: 25px; "><?= $type['UserType'] ?></h2>
                        <div class="all">
                            <div class="col-12">
                            ...
                            <div class="row">
                                <?php foreach ($choices as $choice) { ?>
                                    <div class="col chartcontainer text-center reportschartsfor_<?= $type['Id'] ?>_type_choice_<?= $choice['id'] ?>">
                            ...
                            <div class="row p-2">
                                <?php foreach ($choices as $i => $choice) { ?>
                                    <div class="col text-center pt-2 <?= $i == 0 ? "f-c-r" : ((sizeof($choices) == ($i + 1)) ? 'l-c-r' : '') ?>" style="background-color: <?= $colors[$i] ?>;">
                                        ...
                                        <p class="text-white reportscolfor_<?= $type['Id'] ?>_type_choice_<?= $choice['id'] ?>"><?= $choice['title_en'] ?><br><span>No answers yet</span></p>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
                        <div class="male">
                            ...
                            <div class="row">
                                <?php foreach ($choices as $choice) { ?>
                                    <div class="col chartcontainer text-center reportschartsfor_<?= $type['Id'] ?>_male_choice_<?= $choice['id'] ?>">
                                        <img src="<?= base_url('assets/images/nodatainchart.svg') ?>" class="empty-chart">
                                    </div>
                                <?php } ?>
                            </div>
                        ...
                        <div class="female">
                            ...
                            <div class="row">
                                <?php foreach ($choices as $choice) { ?>
                                    <div class="col chartcontainer text-center reportschartsfor_<?= $type['Id'] ?>_female_choice_<?= $choice['id'] ?>">
                                        <img src="<?= base_url('assets/images/nodatainchart.svg') ?>" class="empty-chart">
                                    </div>
                                <?php } ?>
                            </div>
                        ...
                        <div class=" age">
                            ...
                            <div class="row">
                                <?php foreach ($choices as $choice) { ?>
                                    <div class="col chartcontainer text-center reportschartsfor_<?= $type['Id'] ?>_age_choice_<?= $choice['id'] ?>">
                                        <img src="<?= base_url('assets/images/nodatainchart.svg') ?>" class="empty-chart">
                                    </div>
                                <?php } ?>
                            </div>
...
<?php foreach ($types as $type) { ?>
    <?php foreach ($filters as $filter) { ?>
        <?php $fullfilters = array_merge(['surveyid' => $surveyid, 'usertype' => $type['Id']], $filter['filters']);
        $all = $this->sv_company_reports->ClimateChoices($fullfilters);  ?>
        <?php if (!empty($all)) { ?>
            <?php
            ...
            foreach ($all as $sn => $choice) {  ?>
                <script>
                    var selector = '.reportschartsfor_<?= $type['Id'] ?>_<?= $filter['name'] ?>_choice_<?= $choice['id'] ?>';
                    var colselector = '.reportscolfor_<?= $type['Id'] ?>_<?= $filter['name'] ?>_choice_<?= $choice['id'] ?> span';
                    <?php $labels = array(); ?>
