...
<script>
            ...
            success: function(response) {
                if (response.status == "ok") {
                    ...
                    Object.entries(response.messages).forEach((key, messages) => {
                        if (key[0] !== "") {
                            var chatdate = (key[0] == today ? "Today" : key[0]) || "--/--/----";
    ...
    const darwmessage = (data, type, old = false) => {
        ...
        const template = `<li class="${type == "sent" && "right"} chat-message animate__animated ${type == "sent" ? "animate__bounceInRight" : "animate__bounceInLeft"}" data-key="${data.Id}">
            <div class="conversation-list">
                <div class="ctext-wrap">
                    <div class="ctext-wrap-content">
                        <h5 class="font-size-14 conversation-name text-dark">${type == "sent" ? "You" : "<?= $sessiondata['type'] == "consultant" ? $targetedAccount["Name"] : "The Consultant" ?>"}
