                        ...
                        <ul class="nav nav-tabs nav-justified" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link byclasses" data-toggle="tab" href="#navtabs2-profile" role="tab">
                                    ...
                                    <span class="d-none d-sm-block"> By usertype </span>
                                </a>
                            </li>
                        </ul>
                            ...
                            <div class="tab-pane active" id="navtabs2-home" role="tabpanel">
                                ...
                                <select class="select2 form-control select2-multiple" name="by_user" multiple="multiple" data-placeholder="Choose ...">
                                    <?php foreach ($members as $user) { ?>
                                        <option value="<?= $user['Id'] ?>"><?= $user['F_name_EN'] . " " . $user['L_name_EN'] ?></option>
                                    <?php } ?>
                                </select>
                                <div class="byuser-error text-danger"></div>
                            </div>
                            <div class="tab-pane" id="navtabs2-profile" role="tabpanel">
                                <label for="class"> usertype : </label><br>
                                <select class="select2 form-control select2-multiple " name="types[]" id="class" multiple="multiple" data-placeholder="Choose ...">
                                    <?php foreach ($userstypes as $userstype) { ?>
                                        <option value="<?= $userstype['code'] ?>"><?= $userstype['name'] ?></option>
                                    <?php } ?>
                                </select>
...
<script>
    ...
    $('.showQr').click(function() {
        ...
        if ($('.byuser').hasClass('active')) {
            ...
            if ($('select[name="types[]"]').val().length <= 0) {
                $('.byclasses-error').html("Please select at least one Type.");
            } else {
                var data = {
