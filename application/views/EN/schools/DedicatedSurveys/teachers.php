            ...
            <div class="col-lg-8 offset-lg-1">
                <div class="card">
                    <form class="card-body" method="post" action="<?= current_url(); ?>">
                        ...
                        <?php } ?>
                        <h3 class="card-title text-center mt-3"> Publish Survey : </h3>
                        <hr>
                        <div class="col-lg-12">
                            <div class="mb-3">
                                <label class="form-label">The survey :</label>
                                <select class="form-control select2" name="survey">
                                    <?php foreach ($surveys as $survey) { ?>
                                        <option value="<?= $survey['survey_id'] ?>"><?= $survey['Title_en'] . ' - ' . $survey['set_name_en'] . " ( From " . $survey['From_date'] .", To :".$survey['To_date'].')' ?></option>
                                    <?php } ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12">
                            <div class="mb-3">
                                <label class="form-label">Students from :</label>
...
<script>
    ...
    $('select[name="class"]').change(function() {
        ...
        $('select[name="students[]"]').select2({
            ...
            ajax: {
                url: '<?= base_url("EN/schools/av_students_in_class"); ?>',
                dataType: 'json',
                type: 'POST',
                data: {
                    classId: classid,
