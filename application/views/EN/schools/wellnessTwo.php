        ...
        <div class="card">
            <div class="card-body">
                ...
                <form method="post" class="row">
                            ...
                            <?php if (!empty($our_parents)) { ?>
                                <optgroup label="Parents">
                                    <?php foreach ($our_parents as $parent) { ?>
                                        <option value="parent:<?= $parent['Id'] ?>"><?= $parent['name_en'] ?> </option>
                                    <?php } ?>
                                </optgroup>
                            <?php } ?>
                        </select>
                    ...
                    <div class="col-lg-4">
                        <label for="user"> Select a survey : </label>
                        <select class="form-control" data-placeholder="Choose ..." name="survey">
                            <option value="">No data Found (try to choose an user) </option>
                        </select>
                    </div>
                    <div class="col-lg-4">
                        <label for="Ruser"> Results for : </label>
                        <select class="form-control" data-placeholder="Choose ..." name="results_user">
                            <option value="">No data Found (try to choose an user and survey) </option>
                        </select>
                    </div>
                    <button class="btn btn-block w-100 m-2 btn-primary" type="submit"> Generate Report </button>
                </form>
            </div>
        </div>
        <?php if (!empty($serv_data)) { ?>
...
<script>
    $('select[name="Targetuser"]').change((function() {
        if ($(this).val() !== "") {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>EN/Schools/doneSurveysForUser',
                data: {
                    userData: $(this).val(),
                },
                success: function(data) {
                    if (Object.keys(data).length > 0) {
                        ...
                        $('select[name="survey"]').html('<option value="">No data Found (try to choose another user)</option>');
                    }
                },
                ajaxError: function() {
                    Swal.fire(
    ...
    $('select[name="survey"]').change((function() {
        if ($(this).val() !== "") {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url(); ?>EN/Schools/resultsUsersForDedicatedSurvey',
                data: {
                    survey_id: $(this).val(),
                },
                success: function(data) {
                    if (Object.keys(data).length > 0) {
                        ...
                        $('select[name="results_user"]').append('<option value="">Select a survey</option>');
                        data.forEach(survey => {
                            $('select[name="results_user"]').append('<option value="' + survey.Id + '">' + survey.name + '</option>');
                        });
                    } else {
                        $('select[name="results_user"]').html('<option value="">No data Found (try to choose another survey)</option>');
                    }
                },
                ajaxError: function() {
                    Swal.fire(
