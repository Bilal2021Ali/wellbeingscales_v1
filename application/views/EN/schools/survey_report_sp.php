...
<div class="main-content">
    <div class="page-content"><br>
        <h4 class="card-title"
            style="background: #add138;padding: 10px;color: #1E1E1E;border-radius: 4px;margin-top: -30px;">CHD 070
            Survey Report</h4>
        <div class="row">
            <div class="col-lg-4">
                <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CHD
                    071 Recent Activity</h4>
                <div class="card">
                    <div class="card-body">
                        <div class="float-right">
                            <div class="dropdown"><a class="dropdown-toggle" href="#" id="dropdownMenuButton3"
                                                     data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <span
                                            class="text-muted">
              <?= ucfirst($type_of_user) ?>
              </span> </a></div>
                        </div>
                                    ...
                                    <li class="feed-item">
                                        <p class="text-muted mb-1 font-size-13">
                                            ...
                                            <small class="d-inline-block ml-1">
                                                <?= $user_passed['Finished_at']; ?>
                                            </small></p>
                                        <p class="mt-0 mb-0">User Type:
                                            <?= $user_passed['UserType'] ?>
                                        </p>
                                        <p class="mt-0 mb-0">Finishing Time:
                                            <?= $user_passed['Finish_time'] ?>
                                        </p>
                                    </li>
                                <?php } ?>
                            </ol>
                ...
                <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CHD
                    075 Detailed Survey Report</h4>
                <div class="card bg-primary">
                    <div class="card-body">
                        <div class="row align-items-center">
                            <div class="col-sm-8">
                                <p class="text-white font-size-18"><b>Live View of Survey Tracking</b><i
                                            class="mdi mdi-arrow-right"></i></p>
                                <div class="mt-4"><a
                                            href="<?= base_url("EN/schools/survey_report_view/" . $serv_id); ?>"
                                            class="btn btn-success waves-effect waves-light">Let’s go!</a></div>
                            </div>
                            <div class="col-sm-4">
                                <div class="mt-4 mt-sm-0"><img
                                            src="<?= base_url(); ?>assets/images/setup-analytics-amico.svg"
                ...
                <div class="card">
                    <div class="card-body text-center">
                        <div class="avatar-sm mx-auto mb-4">
                            <div class="avatar-title rounded-circle bg-soft-primary text-primary font-size-20"><i
                        ...
                        <p class="font-size-15">"
                            <?= str_replace(["staff", "teachers", "students", "parents"], ["Staff", "Teacher", "Student", "Parent"], $type_of_user) ?>
                            " Respondents</p>
                    </div>
                </div>
                <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CHD
                    074 Percentage By Survey Response</h4>
                <div class="card" id="donut_chart_card">
                    <div class="card-body" style="height: 460px;display: grid;">
                        <h4 class="card-title mb-4">Percentage by Survey Response<i
                                    class="uil uil-download-alt float-right"
                                    onclick="savechart('donut_chart_card')"></i></h4>
            ...
            <div class="col-lg-4">
                <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CHD
                    073 Counter By User</h4>
                <div class="card" id="radial_chart_card">
                    <?php if ($type_of_user == "Students") { ?>
                        <div class="card-body">
                            <h4 class="card-title mb-4">Users Chart <i class="uil uil-download-alt float-right"
                    ...
                    <?php } else { ?>
                        <div class="card-body text-center"><img
                                    src="<?= base_url('assets/images/profile_studnt.png') ?>" class="w-50" alt=""></div>
                    <?php } ?>
                </div>
                <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CHD
                    076 Percentage by Gender</h4>
                <div class="card" id="genders_chart_card">
                    <div class="card-body">
                        <h4 class="card-title mb-4">Percentage by Gender<i class="uil uil-download-alt float-right"
                                                                           onclick="savechart('genders_chart_card')"></i>
                        </h4>
        ...
        <div class="row">
                            ...
                            <?php foreach ($quastions as $sn => $quastion) { ?>
                                ...
                                <tr>
                                    ...
                                    <td data-toggle="tooltip" data-placement="top" title=""
                                        data-original-title="<?= $quastion['en_title'] ?>" class="text-truncate"
                                        style="max-width: 100px;"><?= $quastion['en_title'] ?></td>
                                    <?php
                                    foreach ($used_choices as $key => $used_choice) {
                                        $use_count = sizeof(array_column($users_passed_survey, 'answerKey')) > 0 ? $this->db->query(" SELECT `sv_st1_answers_values`.`Id` FROM `sv_st1_answers_values` 
                                                ...
                                                WHERE `question_id` = '" . $quastion['q_id'] . "' AND `choice_id` = '" . $used_choice['Id'] . "' AND `answers_data_id` IN (" . implode(',', array_column($users_passed_survey, 'answerKey')) . ") ")->num_rows() : 0;
            ...
            <div class="col-xl-12">
                ...
                <!--end card-->
                <a href="" id="btn-Convert-Html2Image">Click here</a></div>
            <div class="col-xl-12">
                <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CHD
                    079 Counter By Age</h4>
                <div class="card">
                    <div class="card-body">
        ...
        series: [
            ...
            foreach ($users_types as $type) {
                $use_count = sizeof(array_column($users_passed_survey, 'answerKey')) > 0 ? $this->db->query(" SELECT `sv_st1_answers_values`.`Id` FROM `sv_st1_answers_values` 
                        ...
                        WHERE `choice_id` = '" . $choice['Id'] . "' AND `answers_data_id` IN (" . implode(',', array_column($users_passed_survey, 'answerKey')) . ") ")->num_rows() : 0;
                $data[] = $use_count;
            }
            ?> {
