    ...
    <div class="card">
        <div class="card-body">
            <?php
                                $today = date("Y-m-d");
                                $counter = 0;
                                $Ourstaffs1 = $this->db->query("SELECT * FROM l2_staff WHERE 
`Added_By` = '" . $sessiondata['admin_id'] . "' ")->result_array();
                                foreach ($Ourstaffs1 as $staffs) {
                                    $resultsIntable = $this->db->query("SELECT * FROM l2_gateway_result WHERE `UserId` = '" . $staffs['Id'] . "'
AND Created = '" . $today . "' ORDER BY `Id` DESC LIMIT 1")->num_rows();
                                ...
                                ?>
            <h4 class="card-title mb-4">Number of Staff Without Temperature Record:<a href=""> <?= 0; ?></a></h4>
    ...
    <!--end card-->
        ...
        <?php */ ?>
            ...
            <h4 class="card-title" style="background: #add138;padding: 10px;color: #1E1E1E;border-radius: 4px;">CH 033
                Edit, Delete, and Print QR Code for Teachers</h4>
...
<?php */ ?>
                        ...
                        <a href="<?= base_url(); ?>EN/Schools/infos_Card/AllTeachers"
                           style="position: absolute;right: 10px;"> Show All List of Teachers <i
                ...
                <div class="card">
                    <div class="card-body">
                        <table class="table">
                            ...
                            <tbody>
                            <?php foreach ($teacherslist as $sn => $admin) { ?>
                      ...
                      WHERE `For_User` = '" . $admin["Id"] . "' AND `Type_Of_User` = 'Teacher' LIMIT 1 ")->result_array();
                                    ...
                                    <td>
                                        <a href="<?= base_url() ?>EN/schools/UpdateTeacher/<?= $admin['Id']; ?>">
                                            <i class="uil-pen" style="font-size: 25px;" title="Edit"></i> </a> <a
                                            href="<?= base_url() ?>EN/schools/infos_Card/Teacher/<?= $admin['Id']; ?>">
...
<script>
    var options = {
            ...
            title: {
                text: "Monitor Teacher Temperature ",
                align: "left"
            },
            grid: {
                row: {
