<?php
$left = $isEn ? 'left' : 'right';
$right = $isEn ? 'right' : 'left';
?>
    <style>
        .choice {
            margin-top: 10px;
                ...
                <div class="card">
                    <div class="card-body">
                        <h1 class="card text-center"
                            style="background: #add138; padding: 30px;color: #2E1E1E;border-radius: 20px;"><?= $serv_data['set_name_' . strtolower($language)] ?> </h1>
                        <hr>
                        ...
                        <div class="row">
                                ...
                                <div class="card" style="background : #98d077;">
                                    <div class="card-body">
                                        <h3 class="text-white"><img class="img-icon"
                                                                    ...
                                                                    alt=""> <?= __("total_respondents") ?> <span
                                                    class="float-right"><?= sizeof($users_passed_survey); ?> </span>
                                        </h3>
                                        <h3 class="text-white"><img class="img-icon"
                                                                    src="<?= base_url("assets/images/forcharts/Time.png") ?>"
                                                                    alt=""> <?= __("average_time") ?> <span
                                                    class="float-right"><?= $finishing_time_all ?></span></h3>
                                        <h3 class="text-white"><img class="img-icon"
                                                                    src="<?= base_url("assets/images/forcharts/male_counter.png") ?>"
                                                                    alt=""> <?= __("male_respondents") ?> <span
                                        ...
                                        <h3 class="mb-0 text-white"><img class="img-icon"
                                                                         ...
                                                                         alt=""> <?= __("female_respondents") ?> <span
                            ...
                            <div class="col-lg-12 px-5 default-choices-colors"
                                ...
                                <div class="row">
                                    <?php foreach ($used_choices as $sn => $used_choice) { ?>
                                        <div class="text-center col">
                                            <?= $used_choice['choices_' . strtolower($language)] ?>
                                        </div>
                                    <?php } ?>
                                </div>
                                <div class="row">
                                ...
                                <p class="mt-2"><?= __("showing_results_on_the_date_range") ?>
                                    <br><b><?= __("from") ?></b>: <?= $serv_data['From_date'] ?> <b><?= __("to") ?></b>
                                    : <?= $serv_data['To_date'] ?>
                                    <br><b><?= __("category_name") ?>
                                        :</b> <?= $serv_data['Title_' . strtolower($language)] ?>
                                    <br><b><?= __("survey_name") ?>
                                        :</b> <?= $serv_data['set_name_' . strtolower($language)] ?> <br>
                                </p>
                            </div>
                        </div>
                        <div class="row">
                                                    ...
                                                    <a href="#collapse<?= $group_key; ?>" class="text-dark"
                                                       ...
                                                       aria-controls="collapse<?= $group_key; ?>">
                                                        <div class="card-header" id="headingOne">
                                                            <h6 class="m-0">
                                                                <?= $group_quastion['title_' . strtolower($language)]; ?>
                                                        ...
                                                        WHERE `sv_st_questions`.`Group_id` = '" . $group_quastion['Id'] . "' ")->result_array();
                                                            ...
                                                            <?php foreach ($questions as $question_key => $question) { ?>
                                                                ...
                                                                <h3 class="card-title">Q<?= $questions_counter ?>
                                                                    . <?= $question[strtolower($language) . '_title'] ?>
                                                                    <span class="open_more_q float-right"
                                                                          link="<?= base_url($language . '/schools/question_detailed_report/' . $serv_id . "/" . $question['q_id'] . "/" . $group_choices . '/' . md5(time())) ?>"
                                                                ...
                                                                <div class="row p-2 rowOfChoices">
                                                                    <?php if (!empty($users_passed_survey)) { ?>
                                                                        <?php foreach ($choices as $key => $choice) { ?>
                                                                            ...
                                                                            if ($perc_result > 0) { ?>
                                                                                <div data-container="body"
                                                                                     data-toggle="popover"
                                                                                     data-placement="top"
                                                                                     title="<?= $choice['title_' . strtolower($language)]; ?>"
                                                                                     data-content="<?= __("choice_pop_up", ['use_count' => $use_count, 'perc_result' => $perc_result, 'all_count' => $all_count]) ?>"
                                                                        ...
                                                                        <?php } ?>
                                                                    <?php } else { ?>
                                                                        <p class="p-2 text-center w-100 bg-warning text-white br-2 rounded"><?= __("no_answers") ?></p>
                                                                    <?php } ?>
                                                                </div>
                                                            <?php } ?>
                                                        </div>
                                            ...
                                            <div class="card quastions">
                                                <div class="card-body">
                                                    <?php foreach ($static_questions as $question_key => $question) { ?>
                                                        ...
                                                        <?php $totalPerc = 0; ?>
                                                        <h3 class="card-title">Q<?= $questions_counter ?>
                                                            . <?= $question[strtolower($language) . '_title'] ?></h3>
                                                        <div class="row p-2 rowOfChoices">
                                                            <?php if (!empty($users_passed_survey)) { ?>
                                                                <?php foreach ($choices as $key => $choice) { ?>
                                                                    <?php
