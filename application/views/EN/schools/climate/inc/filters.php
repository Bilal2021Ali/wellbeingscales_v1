...
<div class="card">
    <div class="card-body">
        <form class="row filters-form" method="post">
            ...
            <div
                class="<?= (!isset($disabled) || !in_array('category', $disabled)) ? "col-lg-6 col-md-6" : "col-lg-12 col-md-12" ?> col-sm-12 ">
            ...
            <?php if (!isset($disabled) || !in_array('category', $disabled)) { ?>
                    ...
                    <select multiple name="category[]" class="form-control select2">
                        <?php foreach ($filterssource["category"] as $category) { ?>
                            <option <?= in_array($category["category_id"], $filters["category"]) ? "selected" : "" ?>
                                value="<?= $category["category_id"] ?>"><?= $category["Cat_en"] ?></option>
                        <?php } ?>
                    </select>
                </div>
            <?php } ?>
            <div class="col-lg-12 select-container col-md-6 col-sm-12 mt-2">
                <br><Label>By User Type</Label>
            ...
            <div class="col-lg-12 select-container col-md-6 col-sm-12 classes-filter-container">
                <br><Label>By Class</Label>
                <span class="float-right text-primary btn s-all" data-target-select="class[]" style="margin-top: -9px;">Select all</span>
