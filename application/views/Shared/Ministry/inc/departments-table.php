...
<table id="visi_table"
    ...
    <tr>
        <th>#</th>
        <th><?= __("img") ?></th>
        <th><?= __("arabic_title") ?></th>
        <th><?= __("english_title") ?></th>
        <th><?= __("type") ?></th>
        <th><?= __("user_name") ?></th>
        <th><?= __("city") ?></th>
        <th><?= __("edit") ?></th>
    ...
    <?php foreach ($departments as $sn => $department) { ?>
        <tr>
            ...
            <td>
                <a href="<?= base_url($activeLanguage . "/DashboardSystem/UpdateDepartmentData/" . $department['Dept_Id']) ?>">
                    <i class="uil-pen" style="font-size: 25px;"></i>
                </a>
            </td>
            <?php if (!isset($disableStatusControl)) { ?>
                <td>
