                                ...
                                <div class="row">
                                    <div class="col-12">
                                        <div class="mb-3">
                                            ...
                                            <a href="<?= base_url("AR/" .  $this->router->fetch_class() . "/DedicatedSurvey/" . $serv_id); ?>" class="link btn btn-primary btn-lg btn-block waves-effect waves-light mb-1 mt-1">Start the survey</a>
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    $('select[name="Student"]').change(function() {
        if ($(this).val() !== "The current student") {
            $('.link').show();
            $('.link').attr('href', "<?= base_url("AR/" .  $this->router->fetch_class() . "/DedicatedSurvey/" . $serv_id); ?>/" + $(this).val());
        } else {
            $('.link').hide();
            $('.link').attr('href', "<?= base_url("AR/" .  $this->router->fetch_class() . "/DedicatedSurvey/" . $serv_id); ?>");
        }
    });
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data !== "ok") {
                    ...
                    setTimeout(function() {
                        location.href = "<?= base_url("AR/" .  $this->router->fetch_class() . "/DedicatedSurvey/" . $serv_id); ?>";
                    }, 1000);
                }
            },
            ajaxError: function() {
