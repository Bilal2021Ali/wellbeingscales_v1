        ...
        <?php if ($surveys) { ?>
            <div class="row">
                <?php foreach ($surveys as $sn => $survey) { ?>
                    <div class="col-md-4">
                        <a href="<?= base_url("AR/" . $this->router->fetch_class() . "/DedicatedSurvey/" . $survey['survey_id']); ?>">
                            <div class="card notstatic survey_element">
                                <div class="card-body">
                                    <h3 class="card-title"> <?= ($sn + 1) . " . " . $survey['Title_ar']; ?> <span class="mt-1 font-size-10 card-title-desc float-right"><?= $survey['Created_date'] ?></span> </h3>
