                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">تحديث العنوان</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form class="modal-body">
                            <label>العنوان <span class="text-danger">*</span> :</label>
                            <input class="form-control" name="title" placeholder="العنوان الجديد" type="text">
                            <input type="hidden" name="id">
                            <input type="hidden" name="key">
                            <button type="submit" class="btn btn-primary w-100 mt-2">حفظ</button>
                        </form>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6 mt-2">
                  ...
                  <select name="grades[]" multiple class="form-control select2" id="">
                    ...
                    <?php } ?>
                                ...
                                <?php } ?>
                                <div class="tab-pane active mt-2" id="ar" role="tabpanel">
...
<textarea name="en_article" id="en_article" cols="30" rows="10"></textarea><?php */ ?>
                        ...
                        <div class="choose_file">
                            <label for="choose_file">
                                <input accept="application/pdf" type="file"
                                       ...
                                       name="en_image">
                                <span>تحديد الملف</span> </label>
                        </div>
                        <div class="col-12"><img id="ar_image_preview"
                                                 src="<?= base_url("assets/images/Placeholder-Icon.png") ?>"
                                                 class="w-100 placeholder-image" alt="" srcset=""></div>
                        <br>
                        <div class="col-12">
                            <button type="submit"
                                    class="btn btn-primary btn-lg btn-block waves-effect waves-light mb-1">حفظ
                            </button>
                        </div>
                    </div>
                ...
                <div class="col-lg-6 images">
                    <div class="card">
                        <div class="card-body">
                            <h3 class="card-title">قائمة الموارد</h3>
                            <hr>
                            <?php if (!empty($data)) { ?>
                                <div class="col-12">
                                    <div class="card">
                                                    ...
                                                    <?php foreach ($data as $k => $about_us) { ?>
                                                        <tr id="about_<?= $about_us['Id'] ?>">
                                                            ...
                                                            <td><a target="_blank"
                                                                   ...
                                                                   href="<?= base_url("uploads/l3_about_us/" . $about_us['en_image']) ?>">
                                                                    الرابط </a></td>
                                                            <td>
                                                            ...
                                                            <td class="text-center"><i class="uil uil-trash delete"
                                                                                       data-id="<?= $about_us['Id'] ?>"></i> <?php /*?><a href="<?= base_url("AR/schools/edit_about_us/" . $about_us['Id']); ?>"><i
                                                                class="uil uil-edit-alt edit"
                                                                data-id="<?= $about_us['Id'] ?>"></i><?php */ ?></a>
                                                            </td>
                                                        </tr>
                                                    <?php } ?>
                                                    </tbody>
...
<div class="tab-content p-3 text-muted">
    ...
    <script>
        ...
        $("table").on("change", ".about-datepicker", function () {
            ...
            $.ajax({
                type: "POST",
                url: "<?= base_url("AR/schools/about-us-date") ?>",
                data: {
                    target: item,
        ...
        $("#newarticles").on('submit', function (e) {
            ...
            setTimeout(() => {
                $.ajax({
                    ...
                    ajaxError: function () {
                        $('.alert-danger').html("oops!! آسف لقد حدث خطأ!");
                    }
                });
            }, 500);
        });
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response == "ok") {
                                ...
                                Swal.fire({
                                    title: 'error!',
                                    text: 'sorry !! دينا خطأ غير متوقع ، يرجى المحاولة مرة أخرى في وقت لاحق',
                                    icon: 'error'
                                });
                            }
                        }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response === "ok") {
                        ...
                        Swal.fire({
                            title: 'مشكلة!',
                            text: 'نتأيف لدينا مشكلة , المرجو إعادة المحاولة لاحقا',
                            icon: 'error'
                        });
                    }
                }
