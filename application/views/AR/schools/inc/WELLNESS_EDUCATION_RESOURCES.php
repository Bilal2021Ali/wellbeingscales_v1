...
<?php
$categorys = $this->sv_school_reports->usedcategorys($sessiondata['admin_id']);
?>
<div class="col-lg-12 firstview">
    <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CH 055 موارد الإستبيانات - المقالات والصور</h4>
    <div class="row slides_images">
        <div class="col-lg-6" id="slide_img_1">
            <div class="card">
                <div class="card-body text-center">
                    <h3 class="mb-5">الرسوم البيانية</h3>
                    <?php foreach ($categorys as $category) { ?>
                        <?php
                        $files = $this->db->query("SELECT * FROM `st_sv_categorys_resources` WHERE `cat_id` = '" . $category['Id'] . "'  AND `language_resource` = 'AR' ")->result_array();
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h4 class="card-title mt-0"><?= $category['Cat_ar'] ?></h4>
                                <hr>
                                <a class="btn btn-primary waves-effect waves-light takemeto_Infographics" data-view-id="<?= $category['Id']; ?>">عرض</a>
                            </div>
                        </div>
                    <?php } ?>
                </div>
        ...
        <div class="col-lg-6" id="slide_img_1">
            <div class="card">
                <div class="card-body text-center">
                    <h3 class="mb-5">المقالات</h3>
                    <?php foreach ($categorys as $category) { ?>
                        <?php $articles = $this->db->query("SELECT * FROM `st_sv_categorys_articles` WHERE `cat_id` = '" . $category['Id'] . "'  AND `language` = 'ar' ORDER BY `st_sv_categorys_articles`.`Id` DESC  LIMIT 1")->result_array(); ?>
                        <div class="card">
                        <img class="card-img-top img-fluid w-100" src="<?= base_url(isset($articles[0]['img_url']) ? "uploads/articles_files/".$articles[0]['img_url'] : "assets/images/Placeholder-Icon-File.png"); ?>" alt="Card image cap">
                            <div class="card-body">
                                <h4 class="card-title mt-0"><?= $category['Cat_ar'] ?></h4>
                                <hr>
                                <a href="#" class="btn btn-primary waves-effect waves-light takemeto_articles" data-view-id="<?= $category['Id']; ?>">عرض</a>
                            </div>
                        </div>
                    <?php } ?>
                </div>
...
<div class="col-lg-12 detailed_view_Infographics hidden">
    <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CH 055 الرسوم البيانية</h4>
    <div class="row">
        <?php foreach ($categorys as $category) { ?>
            <div class="col-lg-12 detailedDiv hidden" id="detailed_view_Infographics_<?= $category['Id'] ?>">
                <?php $files = $this->db->query("SELECT * FROM `st_sv_categorys_resources` WHERE `cat_id` = '" . $category['Id'] . "'  AND `language_resource` = 'AR' ")->result_array();  ?>
                <div class="card">
                    <div class="card-body text-center">
                        <h3 class="card-title"><?= $category['Cat_ar'] ?></h3>
                        <div class="row">
                            <i class="back uil uil-arrow-right"></i>
                        </div>
                        <div class="row">
...
<div class="col-lg-12 detailed_view_articles hidden">
    <h4 class="card-title" style="background: #fca301;padding: 10px;color: #1E1E1E;border-radius: 4px;">CH 055 المقالات</h4>
    <div class="row">
        <?php foreach ($categorys as $category) { ?>
            <div class="col-lg-12 detailedDiv hidden" id="detailed_view_articles_<?= $category['Id'] ?>">
                <?php $articles = $this->db->query("SELECT * FROM `st_sv_categorys_articles` WHERE `cat_id` = '" . $category['Id'] . "'  AND `language` = 'AR'  ")->result_array();  ?>
                <div class="card">
                    <div class="card-body text-center">
                        <h3 class="card-title"><?= $category['Cat_ar'] ?></h3>
                        <div class="row">
