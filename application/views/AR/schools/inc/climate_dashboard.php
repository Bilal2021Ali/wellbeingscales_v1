...
<?php if (isset($fullpage)) { ?>
    <div class="main-content">
        ...
        <?php } ?>				  
<div class="col-12">
    <br><h4 class="card-title" style="background: linear-gradient(190deg, #7358a3 0%, #7dbce3 100%);padding: 10px;color: #ffffff;border-radius: 4px;margin-top: -30px;">CH 013: تقارير الجو العام</h4>
    <div class="card">
        <div class="card-body">
                        ...
                        <?php foreach ($climate_survyes as $survey) { ?>
                            <div class="climatesurvey item ml-3" data-category-id="<?= $survey["Cat_id"]; ?>">
                                <div class="card text-center">
                                    <div class="card-body">
                                        <h5 class="font-size-12 mb-2 text-dark"><?= $survey['Title_ar']; ?></h5>
                                        <h3 class="card-title">
											<?= $survey['question'] ?>
											</h3>
                                            <div class="GaugeChartContainer" id="GaugeChart_<?= $survey['survey_id'] ?>"></div>
                                        </div>
                                        <span>الأكثر اختيارا </span>
                                        <hr>
                                    </div>
                                    <a id="GaugeChart_action_<?= $survey['survey_id'] ?>" href="<?= base_url('AR/schools/ClimateReports/' . $this->encrypt_url->safe_b64encode($survey['survey_id'])); ?>" class="w-100 btn btn-warning" style="border-radius: 0px;">اظهر التفاصيل <i class="uil uil-arrow-right"></i></a>
                                </div>
                            </div>
                        <?php } ?>
                    </div>
...
<?php if (!empty($climate_survyes)) {
    foreach ($climate_survyes as $climate_survye) {
        $choices = $this->db
            ...
            sv_st_category.Cat_en, sv_st_category.Cat_ar,sv_sets.title_ar AS set_name_en, sv_sets.title_ar AS set_name_ar ')
            ->select('sv_set_template_answers_choices.title_ar , sv_set_template_answers_choices.title_ar')
            ...
            <script>
                ...
                <?php if (!empty($choices)) { ?>
                    ...
                    <?php foreach ($choices as $choice) { ?>
                        choicesbar += '<div class="col text-center"><?= $choice['title_ar'] . " (" .  ($choice['ChooseingTimes'] > 0 ? $choice['ChooseingTimes'] : "لا نتائج") . ")"  ?></div>';
                    <?php } ?>
                    choicesbar += '</div>';
        ...
        <?php } else { ?>
            <script>
                $('#GaugeChart_' + <?= $climate_survye['survey_id'] ?>).html('<div class="w-100 text-center"><img style="max-width: 300px;" class="w-100 emptysurvey" src="<?= base_url("assets/images/nosurveys.svg") ?>"><h6>لا توجد معلومات حاليا !</h6></div>');
            </script>
        <?php } ?>
    <?php } ?>
    <script>
