...
<div class="main-content">
	...
	<div class="row">
			...
			<br><h4 class="card-title" style="background: linear-gradient(190deg, #7358a3 0%, #7dbce3 100%);padding: 10px;color: #ffffff;border-radius: 4px;margin-top: -30px;">REP 045: تقارير المقابلات</h4>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="userspassedsurveyModalLabel">قائمة المستخدمين <span>(15 تجاوز المستخدم 65)</span> : </h5>
        ...
        <div class="modal fade zoom-in" id="user_answers" tabindex="-1" role="dialog" aria-labelledby="user_answersModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="user_answersModalLabel">الإجابات: </h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                                ...
                                <div class="item ml-3">
                                    <div class="card text-center">
                                        <div class="card-body">
                                            ...
                                            <span>رمز الاستبيان:<?= $survey['survey_id'] ?></span>
                                            <h5 class="font-size-16 mb-1"><a href="#" class="text-dark"><?= $survey['Title_ar']; ?></a></h5>
                                            <p class="text-muted mb-2"><?= 'من تاريخ :' . $survey['From_date'] . ' <br> إلى تاريخ :' . $survey['To_date'] ?></p>
                                        </div>
                                        <div class="btn-group" role="group">
                                            <?php if ($survey['report_link'] !== null) { ?>
                                                <a href="<?= base_url('uploads/Category_resources/AR/') . $survey['report_link'] ?>" class="w-100 text-center"><button type="button" class="w-100 btn btn-outline-light text-truncate"><i class="uil uil-user me-1"></i> التقارير </button></a>
                                            <?php } else { ?>
                                                <button data-toggle="modal" data-target="#userspassedsurvey" type="button" data-survey-id="<?= $survey['survey_id'] ?>" class="w-100 btn btn-outline-light text-truncate showanswereuser"><i class="uil uil-users-alt me-1"></i> مستخدمو الإجابات:</button>
                                            <?php } ?>
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    $('body').on('click', '.showanswereuser', function() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>AR/schools/listofansererusersinprivatsurvey',
            data: {
                survey_id: survey_id,
            },
            beforeSend: function() {
            ...
            success: function(data) {
                ...
                if(data.status == "ok"){
                    ...
                    $('#userspassedsurveyModalLabel span').html("("+ data.passed_counter +" تجاوز المستخدم "+ data.all_counter +") ")
                    users.forEach(user => {
                        var newhtml =  '<li class="feed-item user_answered" data-answer-id="'+ user.answer_id +'">';
                            newhtml += '<p class="text-muted mb-1 font-size-13"> المستخدم: '+ user.name +' , انتهى في: <small class="d-inline-block ml-1">'+ user.Finished_at +'</small></p>';
                            newhtml += '<p class="mt-0 mb-0">نوع المستخدم:'+ user.type +' </p>';
                            newhtml += '<p class="mt-0 mb-0">وقت الانتهاء: '+ user.Finish_time +' </p>';
                            newhtml += '</li>';
                ...
                if(data.status == "ok"){
                    ...
                    answers.forEach(answer => {
                        ...
                        var newhtml =  '<li class="feed-item">';
                            newhtml += '<p class="text-muted mb-1 font-size-13">السؤال: ' + sn + ' : </p>';
                            newhtml += '<p class="mt-0 mb-0"> السؤال:: '+ answer.question +' </p>';
                            newhtml += '<p class="mt-0 mb-0"> الإجابة : '+ answer.answer_Value +' </p>';
                            newhtml += '</li>';
                            $('.returnanswerslist').append(newhtml);
                    });
                }else{
                    $('.returnanswerslist').html('sorry , لدينا خطأ');
                }
            },
            ajaxError: function() {
                $('#ResultsTableStudents').css('background-color', '#DB0404');
    ...
    $('.changer').click(function(){
        location.href = "<?= base_url("AR/schools/Private_surveys"); ?>?type=" + $(this).attr('data-type')
    });
</script>