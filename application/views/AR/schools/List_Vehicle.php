...
<body>
    ...
    <section id="modal-1" class="coustom-modal">
        <h3><span id="modaltITLE">قائمة المعلمين</span><i class="uil uil-compress float-right"></i></h3>
        <hr>
        <div class="rel_teachers_list">
            <table class="table teachers_table">
                <thead>
                    <th>#</th>
                    <th>الاسم</th>
                    <th class="place_th">الصف</th>
                    <th class="text-center">التحديد</th>
                </thead>
                <tbody>
    ...
    <div class="main-content">
        <div class="page-content">
            <h4 class="card-title" style="background: linear-gradient(90deg, rgba(116,83,163,1) 0%, rgba(121,195,236,1) 99%);padding: 10px;color: #ffffff;border-radius: 4px;margin-top: -30px;">CH 040: إدارة المركبات | أضف المعلمين والطلاب إلى المركبات</h4>
                                    ...
                                    <tbody>
                                        <?php foreach ($vehicles as $key => $vehicle) { ?>
                                            <tr>
                                                ...
                                                <td class="actions">
                                                    <i class="uil uil-user-plus AddDriver" data-car-key="<?= $vehicle['Id']; ?>"></i>
                                                    <i class="uil uil-constructor ShowDrivers" data-car-key="<?= $vehicle['Id']; ?>"></i>
                                                </td>
                                                <td class="actions">
                                                    <i class="uil uil-chat-bubble-user Addstudents" data-car-key="<?= $vehicle['Id']; ?>"></i>
                                                    <i class="uil uil-notes ShowStudents" data-car-key="<?= $vehicle['Id']; ?>"></i>
                                                </td>
                                                <td class="actions">
                                                    <i class="uil uil-chat-bubble-user Addstaff" data-car-key="<?= $vehicle['Id']; ?>"></i>
                                                    <i class="uil uil-notes ShowStaff" data-car-key="<?= $vehicle['Id']; ?>"></i>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
...
<script>
    ...
    $('.cars').on('click', '.Addstudents', function() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>AR/schools/Vehicles_students_control',
            data: {
                car_key: car_id,
            },
            cache: false,
            success: function(data) {
                ...
                if (data.length > 0) {
                    data.forEach((student, i) => {
                        ...
                        append_data += "<td>غير مضاف</td>";
                        append_data += '<td class="text-center"><i data-student-key="' + student.Id + '" class="uil uil-plus add_as_student"></i></td>';
                        append_data += '</tr>';
                        $('.teachers_table tbody').append(append_data);
                    });
                } else {
    ...
    $('.cars').on('click', '.Addstaff', function() {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>AR/schools/Vehicles_staff_control',
            data: {
                car_key: car_id,
            },
            cache: false,
    ...
    $('.cars').on('click', '.ShowDrivers', function() {
        $('.place_th').html('class');
        $('.coustom-modal , .backdrop').addClass('show');
        $('#modaltITLE').html('List of Drivers');
        car_id = $(this).attr('data-car-key');
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>AR/schools/Vehicles_list',
    ...
    $('.teachers_table').on('click', '.uil-plus', function() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>AR/schools/Vehicles_list',
            data: {
            ...
            success: function(data) {
                if (data == "ok") {
                    Swal.fire(
                        'success',
                        'The Teacher Connected Successfully',
                        'success'
                    );
                    setTimeout(() => {
                        $('.coustom-modal , .backdrop').removeClass('show');
    ...
    $('.teachers_table').on('click', '.add_as_student', function() {
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url(); ?>AR/schools/Vehicles_students_control',
            data: {
            ...
            success: function(data) {
                if (data == "ok") {
                    Swal.fire(
                        'success',
                        'The Student Connected Successfully',
                        'success'
                    );
                    setTimeout(() => {
                        $('.coustom-modal , .backdrop').removeClass('show');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == "ok") {
                    Swal.fire(
                        'success',
                        'The Staff Connected Successfully',
                        'success'
                    );
                    setTimeout(() => {
                        $('.coustom-modal , .backdrop').removeClass('show');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == "ok") {
                    Swal.fire(
                        'success',
                        'The Teacher Deleted Successfully',
                        'success'
                    );
                    setTimeout(() => {
                        $('.coustom-modal , .backdrop').removeClass('show');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == "ok") {
                    Swal.fire(
                        'success',
                        'The Students Deleted Successfully',
                        'success'
                    );
                    setTimeout(() => {
                        $('.coustom-modal , .backdrop').removeClass('show');
