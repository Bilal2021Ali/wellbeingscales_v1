    ...
    <div class="page-content">
        ...
        <?php foreach ($types as $type) { ?>
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <h3 class="card-title"> <?= $type['name_ech'] ?> : </h3>
    ...
    var options = {
        ...
        series: [
            ...
            <?php $use_count = $this->db->query(" SELECT `sv_st1_answers_values`.`Id` FROM `sv_st1_answers_values` 
                    ...
                    AND `choice_id` = '" . $used_choice['Id'] . "'  AND `sv_school_published_surveys`.`By_school` = '" . $sessiondata['admin_id'] . "' ")->num_rows();
            $counts[] = $use_count;
            } ?> {
                name: "<?= $quastion["ar_title"]; ?>",
                data: [<?= implode(',', $counts) ?>]
            },
            <?php } ?>
        ],
