        ...
        <div class="row">
          <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
              <h4 class="mb-0"> مراقبة الزوار </h4>
            </div>
          </div>
        </div>
        <style>
              ...
              <div class="card">
                <div class="card-body">
                  <form action="<?= current_url(); ?>" method="post">
                    <label class="form-label"> التاريخ (من - إلى) :</label>
                    <div class="input-daterange input-group" id="datepicker6" data-date-format="yyyy-mm-dd" data-date-autoclose="true" data-provide="datepicker">
                      <input type="text" class="form-control" name="start" autocomplete="off" placeholder="من" value="<?= $start ?>" />
                      <input type="text" class="form-control" name="end" autocomplete="off" placeholder="إلى" value="<?= $end ?>" />
                    </div>
                  ...
                  <table id="visi_table" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                    <thead>
                      <tr>
                        ...
                        <th> الإسم </th>
                        <th> تحديث بيانات المستخدم </th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php
                      foreach ($visitors_results as $sn => $results) {
                      ...
                      ?>
                        <tr>
                          ...
                          <td class="text-center"><a class="image-popup-no-margins" href="<?php echo base_url("Proxy/get_image?img_url=") . $results['img_uri']; ?>"> <img class="img-fluid mr-2" alt="" src="<?php echo base_url("Proxy/get_image?img_url=") . $results['img_uri']; ?>" width=" 75"> </a></td>
                          <?php boxes_Colors($results['Result'], $dialostics[0], "الحرارة", "C°", "-59px", "Temperature.png"); ?>
                          ...
                          <td class="text-center flex justify-content-center"><a href="<?php echo base_url("AR/Company_Departments/update_visitor/" . $results['UserId']); ?>" class="btn btn-primary btn-rounded waves-effect waves-light">تحديث بيانات المستخدم</a></td>
                        </tr>
                      <?php } ?>
                    </tbody>
                  </table>
...
?>
  <td class="text-center result" style="background: #<?php echo $back_font ?>">
    <h6 style="color: #<?php echo $color_font ?>"><img src="<?php echo base_url("assets/images/icons/" . $img)  ?>" alt="" class="icon__"><?php echo $text ?></h6>
    <strong style="color: #<?php echo $color_font ?>;display : block;margin-top: 18px;"><?php echo $result; ?></strong><span style="color: #<?php echo $color_font ?>;left: <?php echo $left ?>" class="Unit"><?php echo $unit ?>
      </h6>
  </td>
<?php  } ?>
<?php
...
?>
  <td class="text-center result" style="background: #<?= $bkall ?>">
    <h6 style="color: #<?= $color_font_f ?>">
      <div class="row">
        <div class="col-sm-12">
          <h6 style="color: #191919"><img src="<?= base_url("assets/images/icons/" . $img)  ?>" alt="" class="icon__"></img></h6>
        </div>
        <div class="col-sm-6" style="background: #<?= $back_font_f ?>;padding : 10px;"><strong style="color: #<?= $color_font_f ?>;display : block;">
            <?= $result_f; ?>
          </strong></div>
        <div class="col-sm-6" style="background: #<?= $back_font_l ?>;padding : 10px;"><strong style="color: #<?= $color_font_l ?>;display : block;">
            <?= $result_l; ?>
          </strong></div>
      </div>
