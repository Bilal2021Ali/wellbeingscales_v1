                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <h4 class="mb-0"> مراقبة الحجر
                                <?php echo $action == "الحجر المنزلي" ? "المنزلي" :  "الصحي" ?>
                            </h4>
                        </div>
                    </div>
                </div>
...
FROM l2_co_patient
WHERE `l2_co_patient`.`Added_By` = '" . $sessiondata['admin_id'] . "' AND `l2_co_patient`.`Action` = '" . $action . "' ")->result_array();
                    //print_r($Ourstaffs);
...
AND UserType = '" . $type . "' ORDER BY `TimeStamp` DESC LIMIT 1 ")->result_array();
                ...
                foreach ($tbl_prefix as $pref) {
                    $Staff_list[] = data_of_user($pref['UserType'], $sessiondata, $action);
                }
                ...
                // blood
                /*$low_from = $dialostic['low_from'];
      $low_from = $dialostic['from_to'];
      $high2_back_col = $dialostic['high2_back_col'];
      $high2_font_col = $dialostic['high2_font_col'];
      // Glucose
      $high_back_col = $dialostic['hight_back_col'];
      $high_font_col = $dialostic['hight_font_col'];
      // oxigyen 
      $normal_back_col = $dialostic['normal_back_col'];
      $normal_font_col = $dialostic['normal_font_col'];
      // heartRate 
      $low_back_col = $dialostic['low_back_col'];
      $low_font_col = $dialostic['low_font_col'];
      // w 
      $pre_back_col = $dialostic['pre_back_col'];
      $pre_font_col = $dialostic['pre_font_col'];*/
                ?>
                <style>
                    .badge {
                        text-align: center;
                            ...
                            <div class="card-body">
                                ...
                                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                    <thead>
                                        <tr>
                                            <th> الإسم </th>
                                            <th width="14%"> النتيجة </th>
                                            <th class="text-center" width="14%"> ضغط الدم </th>
                                            <th class="text-center" width="14%"> الأكسجين </th>
                                            <th class="text-center" width="14%"> نبض القلب </th>
                                            <th class="text-center" width="14%"> الوزن </th>
                                            <th class="text-center" width="14%"> الجلوكوز </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        ...
                                        ?>
										 ...
										 `Type_Of_User` = '" . $staffsRes['type'] . "' LIMIT 1 ")->result_array();
                                                                                    ?>
                                                        <?php if (empty($avatar)) {  ?>
                                                            <img src="<?php echo base_url(); ?>uploads/avatars/default_avatar.jpg" class="avatar-xs rounded-circle mr-2" alt="...">
                                                        <?php } else { ?>
                                                            <img src="<?php echo base_url(); ?>uploads/co_avatars/<?php echo $avatar[0]['Link']; ?>" class="avatar-xs rounded-circle mr-2" alt="...">
                                                        <?php } ?>
                                                        <h6 class="font-size-15 mb-1 font-weight-normal"><?php echo $staffsRes['Username']; ?></h6>
                                                        <p class="text-muted" style="font-size: 10px;"><?php echo $staffsRes['Creat'] ?> </p>
                                                    </td>
                                                    <?php boxes_Colors($staffsRes['Result'], $dialostics[0], "الحرارة", "C°", "-26px", "Temperature.png"); ?>
                                                    ...
                                                    <?php boxes_Colors($staffsRes['Blood_oxygen'], $dialostics[3], "الأكسجين", "%", "-46px", "Oxygen.png"); ?>
                                                    <?php boxes_Colors($staffsRes['Heart_rate'], $dialostics[4], "نبض القلب  ", "bpm", "-26px", "Puls.png"); ?>
                                                    <?php boxes_Colors($staffsRes['weight'], $dialostics[5], "الوزن", "kg", "-26px", "Wieght.png"); ?>
                                                    <?php boxes_Colors($staffsRes['Glucose'], $dialostics[6], "الجلكوز", "mmol/L", "-26px", "Glucose.png"); ?>
