...
<div class="main-content">
    <div class="page-content">
        <h4 class="card-title" style="background: linear-gradient(90deg, rgba(116,83,163,1) 0%, rgba(121,195,236,1) 99%);padding: 10px;color: #ffffff;border-radius: 4px;margin-top: -30px;"><?= $fulldata['set_name_ar'] ?></h4>
        <div class="col-12 card">
            <div class="card-body">
                <h3 class="card-title">العمر:</h3>
        ...
        <?php foreach ($types as $type) { ?>
            <div class="w-100 reportschartsfor_<?= $type ?>">
                <div class="card">
                    <div class="card-body">
                        <h2 class="card-title" style="background: #52d155; padding: 20px;color: #1E1E1E;border-radius: 25px; "><?= str_replace(['1', '2', '3', '4'], ['الموظفين', 'الطلاب', 'المعلمين', 'أولياء الأمور'], $type) ?></h2>
                        <div class="all">
                            <div class="col-12">
                                <h4 class="text-center w-100 mb-2 " style="background: #add138; padding: 10px;color: #1E1E1E;border-radius: 25px;">حسب نوع المستخدم:</h4>
                            </div>
                            <div class="row">
                            ...
                            <div class="row p-2">
                                <?php foreach ($choices as $i => $choice) { ?>
                                    <div class="col text-center pt-2 <?= $i == 0 ? "f-c-r" : ((sizeof($choices) == ($i + 1)) ? 'l-c-r' : '') ?>" style="background-color: <?= $colors[$i] ?>;">
                                        ...
                                        <p class="text-white reportscolfor_<?= $type ?>_type_choice_<?= $choice['id'] ?>"><?= $choice['title_ar'] ?><br><span>بدون أجوبة حاليا</span></p>
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
                        <div class="male">
                            <div class="col-12">
                                <h4 class="text-center w-100 mb-2" style="background: #add138; padding: 10px;color: #1E1E1E;border-radius: 25px;"> حسب الجنس - الذكور:</h4>
                            </div>
                            <div class="row">
                                <?php foreach ($choices as $choice) { ?>
                        ...
                        <div class="female">
                            <div class="col-12">
                                <h4 class="text-center w-100 mb-2" style="background: #add138; padding: 10px;color: #1E1E1E;border-radius: 25px;">حسب الجنس - إناث:</h4>
                            </div>
                            <div class="row">
                                <?php foreach ($choices as $choice) { ?>
                        ...
                        <div class=" age">
                            <div class="col-12">
                                <h4 class="text-center w-100 mb-2" style="background: #add138; padding: 10px;color: #1E1E1E;border-radius: 25px;">حسب العمر: (<?= $age['from'] ?>-<?= $age['to'] ?> )</h4>
                            </div>
                            <div class="row">
                                <?php foreach ($choices as $choice) { ?>
...
<?php foreach ($types as $type) { ?>
    <?php foreach ($filters as $filter) { ?>
        ...
        $all = $this->sv_ministry_reports->ClimateChoices($fullfilters);  ?>
        <?php if (!empty($all)) { ?>
            <?php
            ...
            foreach ($all as $sn => $choice) {  ?>
                <script>
                    ...
                    $(selector).html('');
                    $(colselector).html('selected <?= $counts[$sn] ?> من أصل <?= array_sum($counts) ?>');
