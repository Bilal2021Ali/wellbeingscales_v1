...
<body>
                                                        ...
                                                        <?php foreach ($questions as $question_key => $question) { ?>
                                                            ...
                                                            <div class="card">
                                                                <div class="card-body quastions-title">
                                                                    <h3 class="card-title"><?= $questions_counter ?> . <?= $question['ar_title'] ?>
                                                                    <span class="open_more_q float-right" link="<?= base_url('AR/DashboardSystem/question_detailed_report/' . $serv_id . "/" . $question['Id'] . "/" . $group_choices . '/' . md5(time())) ?>" target="_blank" rel="noopener noreferrer"><i class="uil uil-analytics"></i></span>
                                                                    </h3>
                                                                </div>
                                                            </div>
                                                            <div class="row">
                                                                <?php foreach ($choices as $key => $choice) { ?>
                                                                    <?php
                                                                    $use_count = $this->db->query(" SELECT Id FROM `sv_st1_answers_values` WHERE `question_id` = '" . $question['Id'] . "' AND `choice_id` = '" . $choice['Id'] . "' ")->num_rows();
                                                                    $all_count = $this->db->query(" SELECT Id FROM `sv_st1_answers_values` WHERE `question_id` = '" . $question['Id'] . "'  ")->num_rows();
                                                                    ?>
                                                                    <div class="col">
                                                                        <div class="card">
                                                                            <div class="card-body">
                                                                                <div class="custom-control custom-radio mr-2">
                                                                                    <label class=""><i class="uil uil-check-circle"></i><?= $choice['title_en'] ?><span class="float-right">(  <?= calc_perc($use_count, $all_count); ?>)
                                                                                            <span class="open_moreD" link="<?= base_url('AR/DashboardSystem/question_choice_report/' . $serv_id . "/" . $choice['Id'] . "/" . $question['Id'] . "/"  . calc_perc($use_count, $all_count) . '/' . md5(time())) ?>" target="_blank" rel="noopener noreferrer"><i class="uil uil-arrow-up-right"></i></span>
                                                                                        </span>
                                                                                    </label>
                                                                                </div>
                                                                            </div>
                                                                ...
                                                                <div class="card">
                                                                    <div class="card-body">
                                                                        <div class="custom-control mr-2">
                                                                            <label class=""><i class="uil uil-check-circle"></i><?= $choice['title_ar'] ?><span class="float-left">
                                                                                    <span class="open_moreD" link="<?= base_url('AR/DashboardSystem/question_choice_report/' . $serv_id . "/" . $choice['Id'] . "/" . $question['Id'] . "/"  . calc_perc($use_count, $all_count) . '/' . md5(time())) ?>" target="_blank" rel="noopener noreferrer">
                                                                                    <i class="uil uil-arrow-up-right"></i>
                                                                                    </span>
                                                                                    (  <?= calc_perc($use_count, $all_count); ?>)
                                                                                </span>
                                                                            </label>
                                                                        </div>
                                                                    </div>
    ...
    <div class="downBar" >
        <div class="header">
            <i class="uil uil-multiply close_down"></i> <p style="margin-top: -25px;" class="text-center">تقرير مفصل</p>
        </div>
