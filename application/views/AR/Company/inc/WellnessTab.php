<?php $users = $this->db->query(" SELECT  MAX(TimeStamp) AS LastCreated , COUNT(Id) AS Total_Count FROM `l2_staff` ")->result_array(); ?>
<style>
    .font-size-70 {
        font-size: 70px;
        margin-bottom: 10px;
...
<div class="col-lg-12">
    <br> <h4 class="card-title" style="background: linear-gradient(90deg, rgba(116,83,163,1) 0%, rgba(121,195,236,1) 99%);padding: 10px;color: #ffffff;border-radius: 4px;margin-top: -30px;"> CHW 036 COUNTER CARDS-ALL SURVEYS</h4>
    <div class="row">
        <div class="col-lg-3">
                                ...
                                <div class="item ml-3">
                                    <div class="card text-center">
                                        <div class="card-body">
                                            <div class="mb-4">
                                                <div class="avatar-title bg-soft-primary rounded-circle text-primary count_in_list">
                                                    <strong class="display-4 m-0 text-primary" style="font-family: 'Almarai';"><?= $survey['use_count']; ?></strong>
                                                </div>
                                            </div>
                                        ...
                                        <div class="btn-group" role="group">
                                            <?php if ($survey['use_count'] > 0) { ?>
                                                <a href="<?= base_url("EN/Company/survey_reports/" . $survey['survey_id']); ?>" class="btn btn-outline-light text-truncate"><i class="uil uil-notes mr-1"></i>Reports</a>
                                            <?php } ?>
                                            <a href="<?= base_url("EN/Company/survey_preview/" . $survey['survey_id']); ?>" target="_blank" class="btn btn-outline-light text-truncate"><i class="uil uil-eye mr-1"></i> Preview </a>
                                        </div>
                                    </div>
                                </div>
                            <?php } ?>
    ...
    <?php foreach ($supported_types as $type) { ?>
        <?php $ages = $this->sv_company_reports->ages($type['typeKey']) ?>
        <?php $matural = $this->sv_company_reports->matural_status($type['typeKey']) ?>
        <?php $counter_of_published_surveys = sizeof($this->sv_company_reports->specific_type_surveys($type['typeKey'])) ?>
        <?php $counter_of_expired_surveys = sizeof($this->sv_company_reports->specific_type_surveys($type['typeKey'] , ['expired' => true])) ?>
        <?php $counter_of_completed_surveys = sizeof($this->sv_company_reports->completed_surveys(['u_type' => $type['typeKey']])) ?>
        <?php $quastions = sizeof($this->sv_company_reports->specific_type_questions($type['typeKey'])) ?>
        <?php $finishing_time = $this->sv_company_reports->timeOfFinishing(['type' => $type['typeKey']]) ?>
        <div class="row">
            <div class="col-12">
                <h4 class="card-title" style="background: #add138;padding: 10px;color: #1E1E1E;border-radius: 4px;">CHW 045 DATA FREQUENCY OF <?= $type['name'] ?></h4>
                <h4 class="card-title bordred_title" style="padding: 30px;background: #0eacd8;border: 2px solid #006a88;font-size: 25px;"> DATA FREQUENCY OF <?= $type['name'] ?> </h4>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-6">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div id="<?= $type['typeKey'] ?>_genders"></div>
                    </div>
                </div>
                <div class="card mt-2">
                    <div class="card-body">
