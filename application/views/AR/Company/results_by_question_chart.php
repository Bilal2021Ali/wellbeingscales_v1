                ...
                <div class="card-body">
                    ...
                    <form class="row" method="post">
                        ...
                        <div class="col-lg-12 school-select-container">
                            <select name="deparments[]" class="form-control select2 school-select" multiple="multiple" data-placeholder="Select a Department">
                                <?php foreach ($deparments as $deparment) { ?>
                                    <option <?= $filters['deparments'] !== null ?  (in_array($deparment['Id'], $filters['deparments']) ? "selected" : "") : "" ?> value="<?= $deparment['Id'] ?>"><?= $deparment['Dept_Name_EN'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-lg-12 mt-2 supported-classes-container">
                            <select name="userstypes[]" id="supported_classes" class="form-control select2" multiple="multiple" data-placeholder="Select an user type">
                                <?php foreach ($userstypes as $userstype) { ?>
                                    <option <?= $filters['userstypes'] !== null ? (in_array($userstype['code'], gettype($filters['userstypes']) == "array" ? $filters['userstypes'] : []) ? "selected" : "") : "" ?> value="<?= $userstype['code'] ?>"><?= $userstype['name'] ?></option>
                                <?php } ?>
                            </select>
                        </div>
                        <div class="col-12">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6">
                            <div class="card" style="background : #98d077;">
                                <div class="card-body">
                                    <?php foreach ($userstypes as $userstype) { ?>
                                        <?php if (isset($byUserType[$userstype['code']])) { ?>
                                                    ...
                                                    <div class="row p-2 rowOfChoices">
                                                        <?php if (!empty($users_passed_survey)) { ?>
                                                            <?php foreach ($choices as $key => $choice) { ?>
                                                                ...
                                                                $all_count = $this->db->query(" SELECT Id FROM `sv_st1_answers_values` WHERE `question_id` = '" . $question['q_id'] . "'  AND `answers_data_id` IN (" . implode(',',  array_column($users_passed_survey, 'answerKey')) . ") ")->num_rows();
                                                                $perc_result = calc_perc($use_count, $all_count); ?>
                                                                <div data-container="body" data-toggle="popover" data-placement="top" title="<?= $choice['title_en'];  ?>" data-html="true" data-content="- <?= $use_count ?> user(s) answered by this choice <br> - The percentage of users who selected this choice:  <?= $perc_result ?>% <br> - The total number of users who answered the survey: <?= $all_count ?>" class="choice text-center p-2 m-0 col text-truncate" style="background-color: <?= $colors[$key]; ?>;" data-perc-total="<?= $totalPerc ?>">
