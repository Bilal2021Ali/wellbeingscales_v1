                            ...
                            <div class="card-body">
                               <div class="row">
								   ...
								   <div class="col-lg-4" style="padding-top: 30px;">
								       ...
								       class="btn btn-outline-primary waves-effect waves-light">Submit</button>
								   </div>
				               </div>
				            </div>
				        </div>
...
function GetDataForUser(){
	var name = $('#name').val();
      $.ajax({
          type: 'POST',
          url: '<?php echo base_url(); ?>AR/Results/getResultsbyname',
          data: {
            name : name,    
          },
          beforeSend: function() {
  ...
  $( function() {
    var availableTags = [
      ...
      "<?php echo $stafff['F_name_AR']." ".$stafff['M_name_EN']." ".$stafff['L_name_EN']; ?>",
	  <?php } ?>		 
	  <?php foreach($teachers_of_this_school as $teach){ ?>	
      "<?php echo $teach['F_name_AR']." ".$teach['M_name_EN']." ".$teach['L_name_EN']; ?>",
	  <?php } ?>	
	  <?php foreach($Stuents_of_this_school as $stud){ ?>	
      "<?php echo $stud['F_name_AR']." ".$stud['M_name_EN']." ".$stud['L_name_EN']; ?>",
	  <?php } ?>	
    ];
    $( "#name" ).autocomplete({
