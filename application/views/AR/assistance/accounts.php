...
<div class="main-content">
    <div class="page-content">
        ...
        <h4 class="card-title"
            ...
            CH 100: الحسابات الفرعية</h4>
        <div class="row">
            <div class="col-12">
                <div class="page-title-box d-flex align-items-center justify-content-between">

                    <?php $this->load->view('AR/assistance/inc/create') ?>
                </div>
            </div>
        </div>
        <div class="row">
                ...
                <div class="col-xl-3 col-sm-6 account-card" data-key="<?= $account['id'] ?>">
                    <div class="card text-center">
                        ...
                        <div class="btn-group" role="group">
                            <a href="<?= base_url('AR/schools/assistance-account/' . $account['id'] . "/permissions") ?>"
                            ...
                            <button type="button" data-key="<?= $account['id'] ?>"
                                ...
                                <?php if (intval($account['status']) === 1) { ?>
                                    <i class="uil uil-check me-1"></i> تفعيل
                                <?php } else { ?>
                                    <i class="uil uil-times me-1"></i> تعطيل
                                <?php } ?>
                            </button>
                        </div>
                    </div>
...
<script>
    $(".edit-profile").click(function () {
        ...
        $.get("<?= base_url('AR/schools/assistance-account/') ?>" + id + "/profile", function (response) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status === 'ok') {
                    ...
                    if (response.to === 1) {
                        $('.account-card[data-key="' + id + '"] .account-status').addClass("text-success bg-soft-success").html('<i class="uil uil-check me-1"></i> تفعيل');
                    } else {
                        $('.account-card[data-key="' + id + '"] .account-status').addClass("text-danger bg-soft-danger").html('<i class="uil uil-times me-1"></i> تعطيل');
                    }
                } else {
                    command: toastr['error'](response.message);
                }
